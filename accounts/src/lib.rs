...
use vast_crypto::publickey::{Address, Message, Public, Secret, Random, Generator, Signature};
use vaststore::accounts_dir::MemoryDirectory;
use vaststore::{
	SimpleSecretStore, SecretStore, VastStore, VastMultiStore,
	random_string, SecretVaultRef, StoreAccountRef, OpaqueSecret,
};
use log::warn;
use parking_lot::RwLock;

pub use vaststore::{Derivation, IndexDerivation, KeyFile, Error};
...
pub struct AccountProvider {
	/// For performance reasons some mvastods can re-use unlocked secrets.
	...
	transient_sstore: VastMultiStore,
...
fn transient_sstore() -> VastMultiStore {
	VastMultiStore::open(Box::new(MemoryDirectory::default())).expect("MemoryDirectory load always succeeds; qed")
}
...
impl AccountProvider {
	...
	pub fn transient_provider() -> Self {
		AccountProvider {
			...
			sstore: Box::new(VastStore::open(Box::new(MemoryDirectory::default())).expect("MemoryDirectory load always succeeds; qed")),
			transient_sstore: transient_sstore(),
	...
	/// Checks whvaster an account with a given address is present.
	...
	/// Helper mvastod used for unlocking accounts.
	...
	pub fn list_gvast_accounts(&self, testnet: bool) -> Vec<Address> {
		self.sstore.list_gvast_accounts(testnet).into_iter().map(|a| Address::from(a).into()).collect()
	}

	/// Returns the underlying `SecretStore` reference if one exists.
	pub fn import_gvast_accounts(&self, desired: Vec<Address>, testnet: bool) -> Result<Vec<Address>, Error> {
		self.sstore.import_gvast_accounts(SecretVaultRef::Root, desired, testnet)
...
mod tests {
	...
	use vast_crypto::publickey::{Generator, Random, Address};
	use vaststore::{StoreAccountRef, Derivation};
	use vast_types::H256;
