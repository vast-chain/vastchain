...
use vaststore::{VastStore, SimpleSecretStore, SecretVaultRef, StoreAccountRef};
use vast_crypto::publickey::{Random, Generator, Secret, KeyPair, verify_address};
...
fn secret_store_create() {
	...
	let _ = VastStore::open(Box::new(dir)).unwrap();
}
...
fn secret_store_create_account() {
	let dir = TransientDir::create().unwrap();
	let store = VastStore::open(Box::new(dir)).unwrap();
...
fn test_path() -> &'static str {
	match ::std::fs::metadata("vaststore") {
		Ok(_) => "vaststore/tests/res/gvast_keystore",
 		Err(_) => "tests/res/gvast_keystore",
	}
}

fn pat_path() -> &'static str {
	match ::std::fs::metadata("vaststore") {
		Ok(_) => "vaststore/tests/res/pat",
		Err(_) => "tests/res/pat",
	}
}
...
fn ciphertext_path() -> &'static str {
	match ::std::fs::metadata("vaststore") {
		Ok(_) => "vaststore/tests/res/ciphertext",
		Err(_) => "tests/res/ciphertext",
	}
}

#[test]
fn secret_store_laod_gvast_files() {
