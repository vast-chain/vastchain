...
use docopt::Docopt;
use vaststore::accounts_dir::{KeyDirectory, RootDiskDirectory};
use vastkey::Password;
use vast_crypto::publickey::Address;
use vaststore::{VastStore, SimpleSecretStore, SecretStore, import_accounts, PresaleWallet, SecretVaultRef, StoreAccountRef};
...
Usage:
    ...
    vaststore import [<password>] [--src DIR] [--dir DIR]
...
enum Error {
	Vaststore(vaststore::Error),
	Docopt(docopt::Error),
}

impl From<vaststore::Error> for Error {
	fn from(err: vaststore::Error) -> Self {
		Error::Vaststore(err)
	}
}
...
impl fmt::Display for Error {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		match *self {
			Error::Vaststore(ref err) => fmt::Display::fmt(err, f),
...
fn key_dir(location: &str, password: Option<Password>) -> Result<Box<dyn KeyDirectory>, Error> {
	let dir: RootDiskDirectory = match location {
		"gvast" => RootDiskDirectory::create(dir::gvast(false))?,
		"gvast-test" => RootDiskDirectory::create(dir::gvast(true))?,
		path if path.starts_with("vast") => {
			let chain = path.split('-').nth(1).unwrap_or("vast");
			let path = dir::vast(chain);
...
fn open_args_vault(store: &VastStore, args: &Args) -> Result<SecretVaultRef, Error> {
...
fn open_args_vault_account(store: &VastStore, address: Address, args: &Args) -> Result<StoreAccountRef, Error> {
...
fn load_password(path: &str) -> Result<Password, Error> {
	let mut file = fs::File::open(path).map_err(|e| vaststore::Error::Custom(format!("Error opening password file '{}': {}", path, e)))?;
	let mut password = String::new();
	file.read_to_string(&mut password).map_err(|e| vaststore::Error::Custom(format!("Error reading password file '{}': {}", path, e)))?;
	// drop EOF
	let _ = password.pop();
	Ok(password.into())
}
...
fn execute<S, I>(command: I) -> Result<String, Error> where I: IntoIterator<Item=S>, S: AsRef<str> {
	...
	let store = VastStore::open(key_dir(&args.flag_dir, None)?)?;

	return if args.cmd_insert {
		let secret = args.arg_secret.parse().map_err(|_| vaststore::Error::InvalidSecret)?;
		...
		let address = args.arg_address.parse().map_err(|_| vaststore::Error::InvalidAccount)?;
		...
		let address = args.arg_address.parse().map_err(|_| vaststore::Error::InvalidAccount)?;
		let message = args.arg_message.parse().map_err(|_| vaststore::Error::InvalidMessage)?;
		let password = load_password(&args.arg_password)?;
		let account_ref = open_args_vault_account(&store, address, &args)?;
