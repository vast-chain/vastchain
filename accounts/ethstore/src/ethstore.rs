...
/// Accounts store.
pub struct VastStore {
	store: VastMultiStore,
}
...
impl VastStore {
	...
	pub fn open_with_iterations(directory: Box<dyn KeyDirectory>, iterations: u32) -> Result<Self, Error> {
		Ok(VastStore {
			store: VastMultiStore::open_with_iterations(directory, iterations)?,
		})
	}
	...
	///
	/// By default refreshing is disabled, so only accounts created using this instance of `VastStore` are taken into account.
...
impl SimpleSecretStore for VastStore {
...
impl SecretStore for VastStore {
	...
	fn list_gvast_accounts(&self, testnet: bool) -> Vec<Address> {
		import::read_gvast_accounts(testnet)
	}

	fn import_gvast_accounts(&self, vault: SecretVaultRef, desired: Vec<Address>, testnet: bool) -> Result<Vec<StoreAccountRef>, Error> {
		let imported_addresses = match vault {
			SecretVaultRef::Root => import::import_gvast_accounts(&*self.store.dir, desired.into_iter().collect(), testnet),
			SecretVaultRef::Vault(vault_name) => {
				if let Some(vault) = self.store.vaults.lock().get(&vault_name) {
					import::import_gvast_accounts(vault.as_key_directory(), desired.into_iter().collect(), testnet)
				} else {
...
/// Similar to `VastStore` but may store many accounts (with different passwords) for the same `Address`
pub struct VastMultiStore {
...
impl SimpleSecretStore for VastMultiStore {
	fn insert_account(&self, vault: SecretVaultRef, secret: Secret, password: &Password) -> Result<StoreAccountRef, Error> {
...
mod tests {
	...
	use super::{VastStore, VastMultiStore};
	...
	fn store() -> VastStore {
		VastStore::open(Box::new(MemoryDirectory::default())).expect("MemoryDirectory always load successfuly; qed")
	}

	fn multi_store() -> VastMultiStore {
		VastMultiStore::open(Box::new(MemoryDirectory::default())).expect("MemoryDirectory always load successfuly; qed")
	}
	...
	fn should_create_and_open_vaults() {
		...
		let store = VastStore::open(dir.key_dir.take().unwrap()).unwrap();
