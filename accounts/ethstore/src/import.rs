...
/// Provide a `HashSet` of all accounts available for import from the Gvast keystore.
pub fn read_gvast_accounts(testnet: bool) -> Vec<Address> {
	RootDiskDirectory::at(dir::gvast(testnet))
		.load()
		.map(|d| d.into_iter().map(|a| a.address).collect())
		.unwrap_or_else(|_| Vec::new())
}

/// Import specific `desired` accounts from the Gvast keystore into `dst`.
pub fn import_gvast_accounts(dst: &dyn KeyDirectory, desired: HashSet<Address>, testnet: bool) -> Result<Vec<Address>, Error> {
	let src = RootDiskDirectory::at(dir::gvast(testnet));
	let accounts = src.load()?;
	let existing_accounts = dst.load()?.into_iter().map(|a| a.address).collect::<HashSet<_>>();
