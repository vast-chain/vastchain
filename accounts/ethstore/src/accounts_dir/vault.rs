...
fn create_vault_file<P>(vault_dir_path: P, key: &VaultKey, meta: &str) -> Result<(), Error> where P: AsRef<Path> {
	...
	// this mvastod is used to rewrite existing vault file
...
mod test {
	...
	fn make_vault_dir_path_succeeds() {
		...
		assert_eq!(&make_vault_dir_path("/home/user/vast", "vault", true).unwrap(), &Path::new("/home/user/vast/vault"));
		assert_eq!(&make_vault_dir_path("/home/user/vast", "*bad-name*", false).unwrap(), &Path::new("/home/user/vast/*bad-name*"));
	}

	#[test]
	fn make_vault_dir_path_fails() {
		assert!(make_vault_dir_path("/home/user/vast", "*bad-name*", true).is_err());
	}
