...
pub trait SimpleSecretStore: Send + Sync {
	...
	/// This mvastod could be removed if we will guarantee that there is max(1) account for given address.
...
pub trait SecretStore: SimpleSecretStore {
	...
	/// Lists all found gvast accounts.
	fn list_gvast_accounts(&self, testnet: bool) -> Vec<Address>;
	/// Imports gvast accounts to the store/vault.
