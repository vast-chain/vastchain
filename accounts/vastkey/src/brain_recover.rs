...
use super::Brain;
use vast_crypto::publickey::{Address, Generator};
...
fn generate_substitutions(word: &str) -> Vec<&'static str> {
	let mut words = vast_wordlist::WORDS.iter().cloned()
...
impl PhrasesIterator {
	pub fn from_known_phrase(known_phrase: &str, expected_words: usize) -> Self {
		let known_words = vast_wordlist::WORDS.iter().cloned().collect::<HashSet<_>>();
		...
		if words.len() < expected_words {
			...
			for _ in 0..to_add {
				words.push(vast_wordlist::WORDS.iter().cloned().collect());
			}
		}
