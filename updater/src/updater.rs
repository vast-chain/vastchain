...
use vast_types::{H256, H160};
use hash_fetch::{self as fetch, HashFetch};
use vast_path::restrict_permissions_owner;
use service::Service;
...
/// Service for checking for updates and determining whvaster we can achieve consensus.
...
impl Updater {
	...
	fn update_file_name(v: &VersionInfo) -> String {
		format!("vast-{}.{}.{}-{:x}", v.version.major, v.version.minor, v.version.patch, v.hash)
	}
}
...
impl<O: OperationsClient, F: HashFetch, T: TimeProvider, R: GenRange> Updater<O, F, T, R> {
	...
	/// Returns release track of the vast node.
	...
	fn updater_step(&self, mut state: MutexGuard<UpdaterState>) {
		...
		if let Some(latest) = state.latest.clone() {
			let fetch = |latest, binary| {
				info!(target: "updater", "Attempting to get vast binary {}", binary);
...
pub mod tests {
	...
	fn should_update_on_new_release() {
		...
		let update_file = tempdir.path().join("vast");
