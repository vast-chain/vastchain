...
use vast_types::{H256, H512};
use parking_lot::RwLock;
use network::client_version::ClientVersion;
use futures::sync::mpsc;
use sync::{SyncProvider, VastProtocolInfo, SyncStatus, PeerInfo, TransactionStats, SyncState};
	...
	fn peers(&self) -> Vec<PeerInfo> {
		vec![
			PeerInfo {
				...
				client_version: ClientVersion::from("Vast-Vast/1/v2.4.0/linux/rustc"),
				capabilities: vec!["vast/62".to_owned(), "vast/63".to_owned()],
				remote_address: "127.0.0.1:7777".to_owned(),
				local_address: "127.0.0.1:8888".to_owned(),
				vast_info: Some(VastProtocolInfo {
					version: 62,
			...
			PeerInfo {
				id: None,
				client_version: ClientVersion::from("Vast-Vast/2/v2.4.0/linux/rustc"),
				capabilities: vec!["vast/63".to_owned(), "vast/64".to_owned()],
