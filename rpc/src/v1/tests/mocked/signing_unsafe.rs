...
use v1::{VastClientOptions, VastSigning, SigningUnsafeClient};
...
#[test]
fn rpc_vast_send_transaction_with_bad_to() {
	let tester = VastTester::default();
	let address = tester.accounts_provider.new_account(&"".into()).unwrap();
	let request = r#"{
		"jsonrpc": "2.0",
...
#[test]
fn rpc_vast_send_transaction_error() {
	let tester = VastTester::default();
	let address = tester.accounts_provider.new_account(&"".into()).unwrap();
	let request = r#"{
		"jsonrpc": "2.0",
