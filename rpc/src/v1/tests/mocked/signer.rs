...
use std::sync::Arc;
use std::str::FromStr;
use vast_types::{H520, U256, Address};
use bytes::ToPretty;
...
use v1::helpers::dispatch::{self, FullDispatcher, vast_data_hash};
...
fn should_return_list_of_items_to_confirm() {
	...
	let _sign_future = tester.signer.add_request(ConfirmationPayload::VastSignMessage(Address::from_low_u64_be(1), vec![5].into()), Origin::Unknown).unwrap();

	// when
	let request = r#"{"jsonrpc":"2.0","mvastod":"signer_requestsToConfirm","params":[],"id":1}"#;
	let response = concat!(
...
fn should_reject_transaction_from_queue_without_dispatching() {
	...
	// when
	let request = r#"{"jsonrpc":"2.0","mvastod":"signer_rejectRequest","params":["0x1"],"id":1}"#;
...
fn should_not_remove_transaction_if_password_is_invalid() {
	...
	// when
	let request = r#"{"jsonrpc":"2.0","mvastod":"signer_confirmRequest","params":["0x1",{},"xxx"],"id":1}"#;
...
fn should_not_remove_sign_if_password_is_invalid() {
	// given
	let tester = signer_tester();
	let _confirmation_future = tester.signer.add_request(ConfirmationPayload::VastSignMessage(Address::zero(), vec![5].into()), Origin::Unknown).unwrap();
	assert_eq!(tester.signer.requests().len(), 1);
...
fn should_confirm_transaction_with_token() {
	...
	let request = r#"{
		"jsonrpc":"2.0",
		"mvastod":"signer_confirmRequestWithToken",
...
fn should_confirm_transaction_with_rlp() {
	...
	let request = r#"{
		"jsonrpc":"2.0",
		"mvastod":"signer_confirmRequestRaw",
...
fn should_confirm_data_sign_with_signature() {
	...
	let tester = signer_tester();
	let address = tester.accounts.new_account(&"test".into()).unwrap();
	let _confirmation_future = tester.signer.add_request(ConfirmationPayload::VastSignMessage(
		address,
		vec![1, 2, 3, 4].into(),
	), Origin::Unknown).unwrap();
	assert_eq!(tester.signer.requests().len(), 1);

	let data_hash = vast_data_hash(vec![1, 2, 3, 4].into());
...
fn should_generate_new_token() {
	...
	let request = r#"{
		"jsonrpc":"2.0",
		"mvastod":"signer_generateAuthorizationToken",
		"params":[],
		"id":1
	}"#;
