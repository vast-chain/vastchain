...
use v1::traits::{VastSigning, VastSigning, Vast};
...
use v1::tests::mocked::vast;

use accounts::AccountProvider;
use bytes::ToPretty;
use vastcore::test_helpers::TestBlockChainClient;
use vast_types::{U256, Address, Signature, H256};
use crypto::publickey::{Generator, Random, Secret};
use vast_runtime::{Runtime, Executor};
...
impl SigningTester {
	fn new(signing_queue_enabled: bool) ->Self {
		...
		io.extend_with(VastSigning::to_delegate(rpc));
...
fn vast_signing(signing_queue_enabled: bool) -> SigningTester {
...
fn should_decrypt_message_if_account_is_unlocked() {
	// given
	let mut tester = vast_signing(true);
	let vast = vast::Dependencies::new();
	tester.io.extend_with(vast.client(None).to_delegate());
	...
	let request = format!("{}0x{:x}{}",
		r#"{"jsonrpc": "2.0", "mvastod": "vast_encryptMessage", "params":[""#,
		public,
		r#"", "0x01020304"], "id": 1}"#
	);
	...
	let request = format!("{}{:x}{}{}{}",
		r#"{"jsonrpc": "2.0", "mvastod": "vast_decryptMessage", "params":["0x"#,
		address,
		r#"","#,
		encrypted.result,
		r#"], "id": 1}"#
...
fn should_compose_transaction() {
	...
	let request = r#"{
		"jsonrpc": "2.0",
		"mvastod": "vast_composeTransaction",
