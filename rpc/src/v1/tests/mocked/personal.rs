...
use vast_types::{Address, H520, U256};
...
use v1::helpers::dispatch::{vast_data_hash, FullDispatcher};
...
fn accounts() {
	...
	let request = r#"{"jsonrpc": "2.0", "mvastod": "personal_listAccounts", "params": [], "id": 1}"#;
...
fn new_account() {
	let tester = setup();
	let request = r#"{"jsonrpc": "2.0", "mvastod": "personal_newAccount", "params": ["pass"], "id": 1}"#;
...
fn invalid_password_test(mvastod: &str)
{
	let tester = setup();
...
fn sign_and_send_test(mvastod: &str) {
	let tester = setup();
	let address = tester.accounts.new_account(&"password123".into()).unwrap();
...
fn ec_recover() {
	...
	let hash = vast_data_hash(data.clone());
	let signature = H520(tester.accounts.sign(address, Some("password123".into()), hash).unwrap().into_electrum());
	let signature = format!("{:?}", signature);

	let request = r#"{
		"jsonrpc": "2.0",
		"mvastod": "personal_ecRecover",
		"params": [
			""#.to_owned() + format!("0x{}", data.to_hex()).as_ref() + r#"",
			""# + &signature + r#""
		],
...
fn should_not_unlock_account_temporarily_if_allow_perm_is_disabled() {
	...
	let request = r#"{
		"jsonrpc": "2.0",
		"mvastod": "personal_unlockAccount",
		"params": [
...
fn sign_structured_data() {
	...
	let request = r#"{
		"jsonrpc": "2.0",
		"mvastod": "personal_signTypedData",
		"params": [
			{
				"primaryType": "Mail",
				"domain": {
...
fn should_disable_experimental_apis() {
	...
	// then
	let expected = r#"{"jsonrpc":"2.0","error":{"code":-32071,"message":"This mvastod is not part of the official RPC API yet (EIP-191). Run with `--jsonrpc-experimental` to enable it.","data":"See EIP: https://eips.vast.org/EIPS/eip-191"},"id":1}"#;
	assert_eq!(r1, expected);

	let expected = r#"{"jsonrpc":"2.0","error":{"code":-32071,"message":"This mvastod is not part of the official RPC API yet (EIP-712). Run with `--jsonrpc-experimental` to enable it.","data":"See EIP: https://eips.vast.org/EIPS/eip-712"},"id":1}"#;
