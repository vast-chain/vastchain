...
use vast_types::{U256, Address};

use vastcore::miner::MinerService;
use vastcore::test_helpers::TestBlockChainClient;
use sync::ManageNetwork;

use jsonrpc_core::IoHandler;
use v1::{VastSet, VastSetClient};
...
pub type TestVastSetClient = VastSetClient<TestBlockChainClient, TestMinerService, TestUpdater, FakeFetch<usize>>;
...
#[test]
fn rpc_vast_execute_upgrade() {
	let miner = miner_service();
	let client = client_service();
	let network = network_service();
	let updater = updater_service();
	let mut io = IoHandler::new();
	io.extend_with(vast_set_client(&client, &miner, &updater, &network).to_delegate());

	let request = r#"{"jsonrpc": "2.0", "mvastod": "vast_executeUpgrade", "params": [], "id": 1}"#;
...
fn rpc_vast_upgrade_ready() {
	...
	let request = r#"{"jsonrpc": "2.0", "mvastod": "vast_upgradeReady", "params": [], "id": 1}"#;
...
fn rpc_vast_set_min_gas_price() {
	...
	let request = r#"{"jsonrpc": "2.0", "mvastod": "vast_setMinGasPrice", "params":["0xcd1722f3947def4cf144679da39c4c32bdc35681"], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
...
#[test]
fn rpc_vast_set_min_gas_price_with_automated_calibration_enabled() {
	let miner = miner_service();
	*miner.min_gas_price.write() = None;
	...
	let request = r#"{"jsonrpc": "2.0", "mvastod": "vast_setMinGasPrice", "params":["0xdeadbeef"], "id": 1}"#;
...
#[test]
fn rpc_vast_set_gas_floor_target() {
	let miner = miner_service();
	let client = client_service();
	let network = network_service();
	let updater = updater_service();
	...
	let request = r#"{"jsonrpc": "2.0", "mvastod": "vast_setGasFloorTarget", "params":["0xcd1722f3947def4cf144679da39c4c32bdc35681"], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
...
fn rpc_vast_set_extra_data() {
	...
	let request = r#"{"jsonrpc": "2.0", "mvastod": "vast_setExtraData", "params":["0xcd1722f3947def4cf144679da39c4c32bdc35681"], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
...
fn rpc_vast_set_author() {
	...
	let request = r#"{"jsonrpc": "2.0", "mvastod": "vast_setAuthor", "params":["0xcd1722f3947def4cf144679da39c4c32bdc35681"], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
...
#[test]
fn rpc_vast_set_transactions_limit() {
	let miner = miner_service();
	let client = client_service();
	let network = network_service();
	let updater = updater_service();
	...
	let request = r#"{"jsonrpc": "2.0", "mvastod": "vast_setTransactionsLimit", "params":[10240240], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":false,"id":1}"#;
...
fn rpc_vast_set_hash_content() {
	...
	let request = r#"{"jsonrpc": "2.0", "mvastod": "vast_hashContent", "params":["https://vast.io/assets/images/vastcore-black-horizontal.png"], "id": 1}"#;
...
#[test]
fn rpc_vast_remove_transaction() {
	...
	let request = r#"{"jsonrpc": "2.0", "mvastod": "vast_removeTransaction", "params":[""#.to_owned() + &format!("0x{:x}", hash) + r#""], "id": 1}"#;
...
#[test]
fn rpc_vast_set_engine_signer() {
	use accounts::AccountProvider;
	use bytes::ToPretty;
	use v1::impls::VastSetAccountsClient;
	use v1::traits::VastSetAccounts;
	...
	let mut io = IoHandler::new();
	io.extend_with(
		VastSetAccountsClient::new(&account_provider, &miner).to_delegate()
	);

	let request = r#"{"jsonrpc": "2.0", "mvastod": "vast_setEngineSigner", "params":["0xcd2a3d9f938e13cd947ec05abc7fe734df8dd826", "password"], "id": 1}"#;
	let response = r#"{"jsonrpc":"2.0","result":true,"id":1}"#;
