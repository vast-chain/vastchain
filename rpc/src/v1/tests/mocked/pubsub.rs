...
#[test]
fn should_subscribe_to_a_mvastod() {
	// given
	...
	// Subscribe
	let request = r#"{"jsonrpc": "2.0", "mvastod": "vast_subscribe", "params": ["hello", []], "id": 1}"#;
	...
	let response =
		r#"{"jsonrpc":"2.0","mvastod":"vast_subscription","params":{"result":"hello","subscription":"0x43ca64edf03768e1"}}"#;
	assert_eq!(res, Some(response.into()));

	let (res, receiver) = receiver.into_future().wait().unwrap();
	let response =
		r#"{"jsonrpc":"2.0","mvastod":"vast_subscription","params":{"result":"world","subscription":"0x43ca64edf03768e1"}}"#;
	assert_eq!(res, Some(response.into()));
