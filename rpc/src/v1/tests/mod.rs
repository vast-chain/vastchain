...
// stored in vastcore/res/vast/tests/.
//
// usage:
...
macro_rules! extract_chain {
	(iter $file:expr) => {{
		const RAW_DATA: &'static [u8] =
			include_bytes!(concat!("../../../../vastcore/res/vast/tests/", $file, ".json"));
		vastjson::test_helpers::blockchain::Test::load(RAW_DATA).unwrap().into_iter()
	}};
