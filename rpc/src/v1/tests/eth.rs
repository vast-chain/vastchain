...
use vastcore::client::{Client, ClientConfig};
use vastcore::miner::Miner;
use spec::{Genesis, Spec, self};
use vastcore::test_helpers;
use verification::VerifierType;
use vast_types::{Address, H256, U256};
use vastjson::test_helpers::blockchain::BlockChain;
use vastjson::spec::ForkSpec;
use io::IoChannel;
...
use v1::impls::{VastClient, VastClientOptions, SigningUnsafeClient};
use v1::metadata::Metadata;
use v1::tests::helpers::{TestSnapshotService, TestSyncProvider, Config};
use v1::traits::{Vast, VastSigning};
...
impl VastTester {
	fn from_chain(chain: &BlockChain) -> Self {
		...
		let tester = if vastjson::test_helpers::blockchain::Engine::NoProof == chain.engine {
	...
	fn from_spec_conf(spec: Spec, config: ClientConfig) -> Self {
		...
		let vast_sign = SigningUnsafeClient::new(
			&signer,
			dispatcher,
		);

		let mut handler = IoHandler::default();
		handler.extend_with(vast_client.to_delegate());
		handler.extend_with(vast_sign.to_delegate());
...
fn vast_get_block() {
	let chain = extract_chain!("BlockchainTests/ValidBlocks/bcGasPricerTest/RPC_API_Test");
	let tester = VastTester::from_chain(&chain);
	let req_block = r#"{"mvastod":"vast_getBlockByNumber","params":["0x0",false],"id":1,"jsonrpc":"2.0"}"#;
...
#[test]
fn vast_get_block_by_hash() {
	let chain = extract_chain!("BlockchainTests/ValidBlocks/bcGasPricerTest/RPC_API_Test");
	let tester = VastTester::from_chain(&chain);

	// We're looking for block number 4 from "RPC_API_Test_Frontier"
	let req_block = r#"{"mvastod":"vast_getBlockByHash","params":["0x75e65fb3bbf5f53afe26dcc72df6a95b0e8ca5f1c450145d8c3915bd0308b75b",false],"id":1,"jsonrpc":"2.0"}"#;
...
#[test]
fn vast_transaction_count() {
	let secret = "8a283037bb19c4fed7b1c569e40c7dcff366165eb869110a1b11532963eb9cb2".parse().unwrap();
	let tester = VastTester::from_spec(Spec::load(&env::temp_dir(), TRANSACTION_COUNT_SPEC).expect("invalid chain spec"));
...
fn starting_nonce_test() {
	let tester = VastTester::from_spec(Spec::load(&env::temp_dir(), POSITIVE_NONCE_SPEC).expect("invalid chain spec"));
...
register_test!(vast_transaction_count_1, verify_transaction_counts, "BlockchainTests/ValidBlocks/bcWalletTest/wallet2outOf3txs");
register_test!(vast_transaction_count_2, verify_transaction_counts, "BlockchainTests/ValidBlocks/bcTotalDifficultyTest/sideChainWithMoreTransactions");
register_test!(vast_transaction_count_3, verify_transaction_counts, "BlockchainTests/ValidBlocks/bcGasPricerTest/RPC_API_Test");
