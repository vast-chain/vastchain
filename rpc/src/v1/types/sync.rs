...
pub struct PeerProtocolsInfo {
	/// Vast protocol information
	pub vast: Option<VastProtocolInfo>,
	/// PIP protocol information.
	pub pip: Option<PipProtocolInfo>,
}

/// Peer Vast protocol information
#[derive(Default, Debug, Serialize)]
pub struct VastProtocolInfo {
	/// Negotiated vast protocol version
	pub version: u32,
...
impl From<sync::VastProtocolInfo> for VastProtocolInfo {
	fn from(info: sync::VastProtocolInfo) -> Self {
		VastProtocolInfo {
	...
	fn from(p: SyncPeerInfo) -> Self {
		PeerInfo {
			...
			protocols: PeerProtocolsInfo {
				vast: p.vast_info.map(Into::into),
