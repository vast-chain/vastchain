...
impl Derive {
	...
	#[cfg(any(test, feature = "accounts"))]
	pub fn to_derivation(self) -> Result<vaststore::Derivation, ConvertError> {
		Ok(match self {
			Derive::Hierarchical(drv) => {
				vaststore::Derivation::Hierarchical({
					let mut members = Vec::<vaststore::IndexDerivation>::new();
					for h in drv {
						if h.index > ::std::u32::MAX as u64 { return Err(ConvertError::IndexOverlfow(h.index)); }
						members.push(match h.d_type {
							DerivationType::Soft => vaststore::IndexDerivation { soft: true, index: h.index as u32 },
							DerivationType::Hard => vaststore::IndexDerivation { soft: false, index: h.index as u32 },
						});
					}
					members
			   })
			...
			Derive::Hash(drv) => {
				match drv.d_type {
					DerivationType::Soft => vaststore::Derivation::SoftHash(drv.hash.into()),
					DerivationType::Hard => vaststore::Derivation::HardHash(drv.hash.into()),
				}
			},
		})
	}
