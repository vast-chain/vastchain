...
use vastcore_private_tx::{TransactionLog as VastTransactionLog, ValidatorLog as VastValidatorLog, PrivateTxStatus as VastStatus};
...
impl From<VastStatus> for Status {
	fn from(c: VastStatus) -> Self {
		match c {
			VastStatus::Created => Status::Created,
			VastStatus::PrivateStateSync => Status::PrivateStateSync,
			VastStatus::PrivateStateSyncFailed => Status::PrivateStateSyncFailed,
			VastStatus::Validating => Status::Validating,
			VastStatus::Deployed => Status::Deployed,
		}
	}
}
...
impl From<VastValidatorLog> for ValidatorLog {
	fn from(r: VastValidatorLog) -> Self {
		ValidatorLog {
			account: r.account,
...
impl From<VastTransactionLog> for PrivateTransactionLog {
	fn from(r: VastTransactionLog) -> Self {
