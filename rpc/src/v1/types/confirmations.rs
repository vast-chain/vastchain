...
impl fmt::Display for ConfirmationPayload {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		match *self {
			...
			ConfirmationPayload::VastSignMessage(ref sign) => write!(f, "{}", sign),
...
/// Vast-prefixed Sign request
#[derive(Debug, Clone, Eq, PartialEq, Hash, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct VastSignRequest {
	/// Address
	pub address: H160,
	/// Hash to sign
	pub data: Bytes,
...
impl From<(H160, Bytes)> for VastSignRequest {
...
impl fmt::Display for VastSignRequest {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		write!(
			f,
			"sign 0x{} with {}",
...
pub enum ConfirmationPayload {
	...
	/// Signature
	#[serde(rename = "sign")]
	VastSignMessage(VastSignRequest),
...
impl From<helpers::ConfirmationPayload> for ConfirmationPayload {
	fn from(c: helpers::ConfirmationPayload) -> Self {
		match c {
			...
			helpers::ConfirmationPayload::VastSignMessage(address, data) => ConfirmationPayload::VastSignMessage(VastSignRequest {
				address,
				data: data.into(),
			}),
...
impl ConfirmationPayload {
	pub fn sender(&self) -> Option<&H160> {
		match *self {
			...
			ConfirmationPayload::VastSignMessage(ref request) => Some(&request.address),
...
mod tests {
	...
	fn should_serialize_sign_confirmation() {
		...
		let request = helpers::ConfirmationRequest {
			id: 15.into(),
			payload: helpers::ConfirmationPayload::VastSignMessage(Address::from_low_u64_be(1), vec![5].into()),
