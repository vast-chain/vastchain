...
use vast_types::{H160, H256, U256, Bloom as H2048};
use serde::ser::Error;
use serde::{Serialize, Serializer};
use types::encoded::Header as VastHeader;
...
impl From<VastHeader> for Header {
	fn from(h: VastHeader) -> Self {
		(&h).into()
	}
}

impl<'a> From<&'a VastHeader> for Header {
	fn from(h: &'a VastHeader) -> Self {
		Header {
			hash: Some(h.hash()),
...
mod tests {
	use std::collections::BTreeMap;
	use vast_types::{H64, H160, H256, U256, Bloom as H2048};
	use serde_json;
