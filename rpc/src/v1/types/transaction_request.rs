...
pub fn format_vaster(i: U256) -> String {
...
impl fmt::Display for TransactionRequest {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		let vast = self.value.unwrap_or_default();
		match self.to {
			Some(ref to) => write!(
				f,
				"{} VAST from {} to 0x{:?}",
				Colour::White.bold().paint(format_vaster(vast)),
			...
			None => write!(
				f,
				"{} VAST from {} for contract creation",
				Colour::White.bold().paint(format_vaster(vast)),
				Colour::White.bold().paint(
					self.from.as_ref()
						.map(|f| format!("0x{:?}", f))
...
mod tests {
	...
	#[test]
	fn test_format_vaster() {
		assert_eq!(&format_vaster(U256::from(1000000000000000000u64)), "1");
		assert_eq!(&format_vaster(U256::from(500000000000000000u64)), "0.5");
		assert_eq!(&format_vaster(U256::from(50000000000000000u64)), "0.05");
		assert_eq!(&format_vaster(U256::from(5000000000000000u64)), "0.005");
		assert_eq!(&format_vaster(U256::from(2000000000000000000u64)), "2");
		assert_eq!(&format_vaster(U256::from(2500000000000000000u64)), "2.5");
		assert_eq!(&format_vaster(U256::from(10000000000000000000u64)), "10");
	}
}
