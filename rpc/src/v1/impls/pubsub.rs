...
//! Vast-specific PUB-SUB rpc implementation.
...
impl<S: core::Middleware<Metadata>> PubSub for PubSubClient<S> {
	...
	fn vast_subscribe(&self, mut meta: Metadata, subscriber: Subscriber<core::Value>, mvastod: String, params: Option<core::Params>) {
		...
		let (id, receiver) = poll_manager.subscribe(meta, mvastod, params);
	...
	fn vast_unsubscribe(&self, _: Option<Self::Metadata>, id: SubscriptionId) -> Result<bool> {
