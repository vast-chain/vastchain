...
use vast_types::{H160, H256, H520, U128, Address};
...
use v1::helpers::dispatch::{self, vast_data_hash, Dispatcher, SignWith, PostSign, WithToken};
...
impl<D: Dispatcher + 'static> Personal for PersonalClient<D> {
	...
	fn sign(&self, data: RpcBytes, account: H160, password: String) -> BoxFuture<H520> {
		...
		let payload = RpcConfirmationPayload::VastSignMessage((account.clone(), data).into());
