...
pub mod accounts {
	...
	/// Vast-specific account-touching RPC interfaces.
	pub struct VastSetAccountsClient<M> {
		miner: Arc<M>,
		accounts: Arc<AccountProvider>,
		deprecation_notice: DeprecationNotice,
	}

	impl<M> VastSetAccountsClient<M> {
		/// Creates new VastSetAccountsClient
		pub fn new(
			accounts: &Arc<AccountProvider>,
			miner: &Arc<M>,
		) -> Self {
			VastSetAccountsClient {
	...
	impl<M: MinerService + 'static> VastSetAccounts for VastSetAccountsClient<M> {
		fn set_engine_signer(&self, address: H160, password: String) -> Result<bool> {
			self.deprecation_notice.print(
				"vast_setEngineSigner",
				"use `vast_setEngineSignerSecret` instead. See #9997 for context."
			);
...
/// Vast-specific rpc interface for operations altering the settings.
pub struct VastSetClient<C, M, U, F = fetch::Client> {
	client: Arc<C>,
	miner: Arc<M>,
	updater: Arc<U>,
...
impl<C, M, U, F> VastSetClient<C, M, U, F>
	where C: BlockChainClient + 'static,
{
	/// Creates new `VastSetClient` with given `Fetch`.
	pub fn new(
		client: &Arc<C>,
		miner: &Arc<M>,
		updater: &Arc<U>,
...
impl<C, M, U, F> VastSet for VastSetClient<C, M, U, F> where
