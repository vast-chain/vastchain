...
//! Vast RPC interface for the light client.
...
use vast_types::{Address, H64, H160, H256, U64, U256};
use hash::{KECCAK_NULL_RLP, KECCAK_EMPTY_LIST_RLP};
use parking_lot::{RwLock, Mutex};
use rlp::Rlp;
use types::transaction::SignedTransaction;
use types::encoded;
use types::filter::Filter as VastcoreFilter;
use types::ids::BlockId;

use v1::impls::vast_filter::Filterable;
...
use v1::types::{
	...
	SyncInfo as RpcSyncInfo, Transaction, CallRequest, Index, Filter, Log, Receipt, Work, VastAccount
};
...
/// Light client `VAST` (and filter) RPC.
pub struct VastClient<C, S: LightSyncProvider + LightNetworkDispatcher + 'static, OD: OnDemandRequester + 'static> {
	sync: Arc<S>,
	client: Arc<C>,
	on_demand: Arc<OD>,
...
impl<C, S, OD> Clone for VastClient<C, S, OD>
where
...
impl<C, S, OD> VastClient<C, S, OD>
where
	C: LightChainClient + 'static,
	S: LightSyncProvider + LightNetworkDispatcher + ManageNetwork + 'static,
	OD: OnDemandRequester + 'static
{
	/// Create a new `VastClient` with a handle to the light sync instance, client,
...
impl<C, S, OD> Vast for VastClient<C, S, OD>
where
	C: LightChainClient + 'static,
	...
	OD: OnDemandRequester + 'static
{
	...
	fn accounts(&self) -> Result<Vec<H160>> {
		self.deprecation_notice.print("vast_accounts", deprecated::msgs::ACCOUNTS);
	...
	fn transaction_by_hash(&self, hash: H256) -> BoxFuture<Option<Transaction>> {
		...
		if in_txqueue {
			...
			// `vast_getTransactionByHash` more than once to ensure the `txqueue` is up to `date` when it is called
	...
	fn transaction_receipt(&self, hash: H256) -> BoxFuture<Option<Receipt>> {
		...
		Box::new(fetcher.transaction_by_hash(hash).and_then(move |tx| {
			...
			// already been checked for canonicality and whvaster it contains
			// the transaction.
			match tx {
	...
	fn proof(&self, _address: H160, _values:Vec<H256>, _num: Option<BlockNumber>) -> BoxFuture<VastAccount> {
		Box::new(future::err(errors::unimplemented(None)))
	}
...
// This trait implementation triggers a blanked impl of `VastFilter`.
impl<C, S, OD> Filterable for VastClient<C, S, OD>
where
	C: LightChainClient + 'static,
	S: LightSyncProvider + LightNetworkDispatcher + ManageNetwork + 'static,
	OD: OnDemandRequester + 'static
{
	...
	fn logs(&self, filter: VastcoreFilter) -> BoxFuture<Vec<Log>> {
		Box::new(self.fetcher().logs(filter)) as BoxFuture<_>
	}

	fn pending_logs(&self, _block_number: u64, _filter: &VastcoreFilter) -> Vec<Log> {
	...
	fn removed_logs(&self, _block_hash: ::vast_types::H256, _filter: &VastcoreFilter) -> (Vec<Log>, u64) {
		(Default::default(), 0)
	}
}
