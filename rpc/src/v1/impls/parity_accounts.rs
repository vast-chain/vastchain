...
use vast_types::{Address, H160, H256, H520};
use vastkey::{Brain, Password};
use crypto::publickey::{Generator, Secret};
use vaststore::KeyFile;
use accounts::AccountProvider;
use jsonrpc_core::Result;
use v1::helpers::deprecated::{self, DeprecationNotice};
use v1::helpers::errors;
use v1::traits::{VastAccounts, VastAccountsInfo};
use v1::types::{Derive, DeriveHierarchical, DeriveHash, ExtAccountInfo, AccountInfo};

/// Account management (personal) rpc implementation.
pub struct VastAccountsClient {
	accounts: Arc<AccountProvider>,
	deprecation_notice: DeprecationNotice,
}

impl VastAccountsClient {
...
impl VastAccountsClient {
	fn deprecation_notice(&self, mvastod: &'static str) {
		self.deprecation_notice.print(mvastod, deprecated::msgs::ACCOUNTS);
	}
}

impl VastAccountsInfo for VastAccountsClient {
	fn accounts_info(&self) -> Result<BTreeMap<H160, AccountInfo>> {
		self.deprecation_notice("vast_accountsInfo");
	...
	fn default_account(&self) -> Result<H160> {
		self.deprecation_notice("vast_defaultAccount");
...
impl VastAccounts for VastAccountsClient {
	...
	fn new_account_from_phrase(&self, phrase: String, pass: Password) -> Result<H160> {
		self.deprecation_notice("vast_newAccountFromPhrase");
	...
	fn new_account_from_wallet(&self, json: String, pass: Password) -> Result<H160> {
		self.deprecation_notice("vast_newAccountFromWallet");
	...
	fn new_account_from_secret(&self, secret: H256, pass: Password) -> Result<H160> {
		self.deprecation_notice("vast_newAccountFromSecret");
	...
	fn test_password(&self, account: H160, password: Password) -> Result<bool> {
		self.deprecation_notice("vast_testPassword");
	...
	fn change_password(&self, account: H160, password: Password, new_password: Password) -> Result<bool> {
		self.deprecation_notice("vast_changePassword");
		let account: Address = account.into();
		self.accounts
	...
	fn kill_account(&self, account: H160, password: Password) -> Result<bool> {
		self.deprecation_notice("vast_killAccount");
		let account: Address = account.into();
		self.accounts
	...
	fn remove_address(&self, addr: H160) -> Result<bool> {
		self.deprecation_notice("vast_removeAddresss");
	...
	fn set_account_name(&self, addr: H160, name: String) -> Result<bool> {
		self.deprecation_notice("vast_setAccountName");
		let addr: Address = addr.into();
	...
	fn set_account_meta(&self, addr: H160, meta: String) -> Result<bool> {
		self.deprecation_notice("vast_setAccountMeta");
		let addr: Address = addr.into();
	...
	fn import_gvast_accounts(&self, addresses: Vec<H160>) -> Result<Vec<H160>> {
		self.deprecation_notice("vast_importGvastAccounts");
		self.accounts
			.import_gvast_accounts(into_vec(addresses), false)
			.map(into_vec)
			.map_err(|e| errors::account("Couldn't import Gvast accounts", e))
	}

	fn gvast_accounts(&self) -> Result<Vec<H160>> {
		self.deprecation_notice("vast_listGvastAccounts");
		Ok(into_vec(self.accounts.list_gvast_accounts(false)))
	}

	fn create_vault(&self, name: String, password: Password) -> Result<bool> {
		self.deprecation_notice("vast_newVault");
	...
	fn open_vault(&self, name: String, password: Password) -> Result<bool> {
		self.deprecation_notice("vast_openVault");
	...
	fn close_vault(&self, name: String) -> Result<bool> {
		self.deprecation_notice("vast_closeVault");
	...
	fn list_vaults(&self) -> Result<Vec<String>> {
		self.deprecation_notice("vast_listVaults");
	...
	fn list_opened_vaults(&self) -> Result<Vec<String>> {
		self.deprecation_notice("vast_listOpenedVaults");
	...
	fn change_vault_password(&self, name: String, new_password: Password) -> Result<bool> {
		self.deprecation_notice("vast_changeVaultPassword");
	...
	fn change_vault(&self, address: H160, new_vault: String) -> Result<bool> {
		self.deprecation_notice("vast_changeVault");
		self.accounts
	...
	fn get_vault_meta(&self, name: String) -> Result<String> {
		self.deprecation_notice("vast_getVaultMeta");
	...
	fn set_vault_meta(&self, name: String, meta: String) -> Result<bool> {
		self.deprecation_notice("vast_setVaultMeta");
	...
	fn derive_key_index(&self, addr: H160, password: Password, derivation: DeriveHierarchical, save_as_account: bool) -> Result<H160> {
		self.deprecation_notice("vast_deriveAddressIndex");
		let addr: Address = addr.into();
		self.accounts
			.derive_account(
				&addr,
	...
	fn derive_key_hash(&self, addr: H160, password: Password, derivation: DeriveHash, save_as_account: bool) -> Result<H160> {
		self.deprecation_notice("vast_deriveAddressHash");
		let addr: Address = addr.into();
		self.accounts
			.derive_account(
				&addr,
	...
	fn export_account(&self, addr: H160, password: Password) -> Result<KeyFile> {
		self.deprecation_notice("vast_exportAccount");
		let addr = addr.into();
		self.accounts
			.export_account(
				&addr,
	...
	fn sign_message(&self, addr: H160, password: Password, message: H256) -> Result<H520> {
		self.deprecation_notice("vast_signMessage");
		self.accounts
			.sign(
				addr.into(),
				Some(password),
