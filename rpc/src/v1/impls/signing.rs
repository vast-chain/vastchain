...
use vast_types::{H160, H256, H520, U256};
...
use v1::traits::{VastSigning, VastSigning};
use v1::types::{
	Bytes as RpcBytes,
	Either as RpcEither,
...
impl<D: Dispatcher + 'static> VastSigning for SigningQueueClient<D> {
	...
	fn post_sign(&self, meta: Metadata, address: H160, data: RpcBytes) -> BoxFuture<RpcEither<U256, RpcConfirmationResponse>> {
		self.deprecation_notice.print("vast_postSign", deprecated::msgs::ACCOUNTS);
		let executor = self.executor.clone();
		let confirmations = self.confirmations.clone();

		Box::new(self.dispatch(
			RpcConfirmationPayload::VastSignMessage((address, data).into()),
			meta.origin
	...
	fn post_transaction(&self, meta: Metadata, request: RpcTransactionRequest) -> BoxFuture<RpcEither<U256, RpcConfirmationResponse>> {
		self.deprecation_notice.print("vast_postTransaction", deprecated::msgs::ACCOUNTS);
		let executor = self.executor.clone();
		let confirmations = self.confirmations.clone();
	...
	fn check_request(&self, id: U256) -> Result<Option<RpcConfirmationResponse>> {
		self.deprecation_notice.print("vast_checkRequest", deprecated::msgs::ACCOUNTS);
	...
	fn decrypt_message(&self, meta: Metadata, address: H160, data: RpcBytes) -> BoxFuture<RpcBytes> {
		self.deprecation_notice.print("vast_decryptMessage", deprecated::msgs::ACCOUNTS);
...
impl<D: Dispatcher + 'static> VastSigning for SigningQueueClient<D> {
	...
	fn sign(&self, meta: Metadata, address: H160, data: RpcBytes) -> BoxFuture<H520> {
		self.deprecation_notice.print("vast_sign", deprecated::msgs::ACCOUNTS);
		let res = self.dispatch(
			RpcConfirmationPayload::VastSignMessage((address, data).into()),
			meta.origin,
		);
	...
	fn send_transaction(&self, meta: Metadata, request: RpcTransactionRequest) -> BoxFuture<H256> {
		self.deprecation_notice.print("vast_sendTransaction", deprecated::msgs::ACCOUNTS);
		let res = self.dispatch(
	...
	fn sign_transaction(&self, meta: Metadata, request: RpcTransactionRequest) -> BoxFuture<RpcRichRawTransaction> {
		self.deprecation_notice.print("vast_signTransaction", deprecated::msgs::ACCOUNTS);
