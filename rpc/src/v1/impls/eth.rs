...
use rlp::Rlp;
use vast_types::{Address, H64, H160, H256, U64, U256, BigEndianHash};
use parking_lot::Mutex;

use account_state::state::StateInfo;
use client_traits::{BlockChainClient, StateClient, ProvingBlockChainClient, StateOrBlock};
use vastash::{self, SeedHashCompute};
use vastcore::client::{Call, EngineInfo};
...
use types::{
	BlockNumber as VastBlockNumber,
	client_types::StateResult,
	encoded,
	header::Header,
	ids::{BlockId, TransactionId, UncleId},
	filter::Filter as VastcoreFilter,
...
use v1::types::{
	...
	Transaction, CallRequest, Index, Filter, Log, Receipt, Work, VastAccount, StorageProof,
	block_number_to_id
};
...
impl VastClientOptions {
	/// Creates new default `VastClientOptions` and allows alterations
...
impl Default for VastClientOptions {
	fn default() -> Self {
		VastClientOptions {
...
/// Vast rpc implementation.
pub struct VastClient<C, SN: ?Sized, S: ?Sized, M, EM> where
...
pub fn base_logs<C, M, T: StateInfo + 'static> (client: &C, miner: &M, filter: Filter) -> BoxFuture<Vec<Log>> where
	...
	let filter: VastcoreFilter = match filter.try_into() {
		Ok(value) => value,
...
impl<C, SN: ?Sized, S: ?Sized, M, EM, T> VastClient<C, SN, S, M, EM> where
...
pub fn pending_logs<M>(miner: &M, best_block: VastBlockNumber, filter: &VastcoreFilter) -> Vec<Log> where M: MinerService {
...
impl<C, SN: ?Sized, S: ?Sized, M, EM, T: StateInfo + 'static> Vast for VastClient<C, SN, S, M, EM> where
	...
	EM: ExternalMinerService + 'static,
{
	...
	fn proof(&self, address: H160, values: Vec<H256>, num: Option<BlockNumber>) -> BoxFuture<VastAccount> {
		...
		let res = match self.client.prove_account(key1, id) {
			Some((proof, account)) => Ok(VastAccount {
				address,
