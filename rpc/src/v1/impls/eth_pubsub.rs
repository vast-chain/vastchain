...
//! Vast PUB-SUB rpc implementation.
...
use types::{
	...
	ids::BlockId,
	encoded,
	filter::Filter as VastFilter,
};

type Client = Sink<pubsub::Result>;

/// Vast PubSub implementation.
pub struct VastPubSubClient<C> {
	handler: Arc<ChainNotificationHandler<C>>,
	heads_subscribers: Arc<RwLock<Subscribers<Client>>>,
	logs_subscribers: Arc<RwLock<Subscribers<(Client, VastFilter)>>>,
	transactions_subscribers: Arc<RwLock<Subscribers<Client>>>,
	sync_subscribers: Arc<RwLock<Subscribers<Client>>>,
}

impl<C> VastPubSubClient<C>
	where
		C: 'static + Send + Sync
{
	...
	OD: OnDemandRequester + 'static
{
	...
	pub fn light(
		...
		VastPubSubClient::new(Arc::new(fetch), executor, pool_receiver)
	}
}
...
pub trait LightClient: Send + Sync {
	...
	/// Fetch logs.
	fn logs(&self, filter: VastFilter) -> BoxFuture<Vec<Log>>;
}
...
impl<C: Send + Sync + 'static> VastPubSub for VastPubSubClient<C> {
