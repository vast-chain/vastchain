...
//! Vast Filter RPC implementation
...
use v1::impls::vast::pending_logs;

/// Somvasting which provides data that can be filtered over.
pub trait Filterable {
	...
	fn pending_logs(&self, block_number: u64, filter: &VastcoreFilter) -> Vec<Log>;
	...
	fn removed_logs(&self, block_hash: H256, filter: &VastcoreFilter) -> (Vec<Log>, u64);
}

/// Vast filter rpc implementation for a full node.
pub struct VastFilterClient<C, M> {
	client: Arc<C>,
	miner: Arc<M>,
	polls: Mutex<PollManager<SyncPollFilter>>,
}

impl<C, M> VastFilterClient<C, M> {
	/// Creates new Vast filter client.
...
impl<C, M> Filterable for VastFilterClient<C, M> where
...
impl<T: Filterable + Send + Sync + 'static> VastFilter for T {
