...
use vast_types::{Address, H160, H256, H520, U256};
...
impl<D: Dispatcher + 'static> VastSigning for SigningUnsafeClient<D>
{
	...
	fn sign(&self, _: Metadata, address: H160, data: RpcBytes) -> BoxFuture<H520> {
		...
		Box::new(self.handle(RpcConfirmationPayload::VastSignMessage((address, data).into()), address)
			.then(|res| match res {
