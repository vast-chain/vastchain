...
use vast_types::{Address, U256};
use hash::H256;
use parking_lot::{Mutex, RwLock};
use fastmap::H256FastMap;
use types::transaction::{Action, Transaction as VastTransaction, PendingTransaction, SignedTransaction, LocalizedTransaction};
	...
	OD: OnDemandRequester + 'static
{
	...
	pub fn proved_read_only_execution(
		...
		Box::new(nonce_fut.join(gas_price_fut).and_then(move |(nonce, gas_price)| {
			future::done(
				Ok((req.gas.is_some(), VastTransaction {
	...
	pub fn logs_no_tx_hash(&self, filter: VastcoreFilter) -> impl Future<Item = Vec<Log>, Error = Error> + Send {
	...
	pub fn logs(&self, filter: VastcoreFilter) -> impl Future<Item = Vec<Log>, Error = Error> + Send {
		use jsonrpc_core::futures::stream::{self, Stream};
