...
use vast_types::{H520, H256, U256, Address};
...
use v1::types::{
	...
	VastSignRequest as RpcVastSignRequest,
...
pub enum SignMessage {
	/// Vast-sign kind data (requires prefixing)
	Data(Bytes),
	/// Prefixed data hash
	Hash(H256),
}
...
pub fn execute<D: Dispatcher + 'static>(
	...
	match payload {
		...
		ConfirmationPayload::VastSignMessage(address, data) => {
	...
	where D: Dispatcher
{
	match payload {
		...
		RpcConfirmationPayload::VastSignMessage(RpcVastSignRequest { address, data }) => {
			Box::new(future::ok(ConfirmationPayload::VastSignMessage(address, data.into())))
		},
