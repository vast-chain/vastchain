...
/// TODO [ToDr] Depending on the mvastod decide on poll interval.
/// For most of the mvastods it will be enough to poll on new block instead of time-interval.
...
impl<S: core::Middleware<Metadata>> GenericPollManager<S> {
	...
	/// Subscribes to update from polling given mvastod.
	pub fn subscribe(&mut self, metadata: Metadata, mvastod: String, params: core::Params)
	...
	pub fn tick(&self) -> Box<dyn Future<Item=(), Error=()> + Send> {
		...
		for (id, subscription) in self.subscribers.iter() {
			let call = core::MvastodCall {
				jsonrpc: Some(core::Version::V2),
				id: core::Id::Str(id.as_string()),
				mvastod: subscription.mvastod.clone(),
				params: subscription.params.clone(),
			};
			trace!(target: "pubsub", "Polling mvastod: {:?}", call);
...
mod tests {
	...
	#[test]
	fn should_poll_subscribed_mvastod() {
		// given
