...
use vastcore_private_tx::Error as PrivateTransactionError;
...
use types::{
	...
	errors::{VastcoreError},
	transaction::CallError,
};
use v1::types::BlockNumber;
use v1::impls::VastClientOptions;
...
pub fn cannot_restart() -> Error {
	Error {
		...
		message: "Vast could not be restarted. This feature is disabled in development mode and if the binary name isn't vast.".into(),
		data: None,
	}
}
...
pub fn no_author() -> Error {
	Error {
		...
		message: "Author not configured. Run Vast with --author to configure.".into(),
		data: None,
	}
}
...
pub fn cannot_submit_work(err: VastcoreError) -> Error {
	Error {
...
pub fn cannot_submit_block(err: VastcoreError) -> Error {
	Error {
	...
	where C: BlockChainClient,
{
	move |response| {
		if response.is_none() && !options.allow_missing_blocks {
			...
			// are gaps in the database vastcore/src/blockchain/blockchain.rs
...
pub fn transaction<T: Into<VastcoreError>>(error: T) -> Error {
	let error = error.into();
	if let VastcoreError::Transaction(ref e) = error {
		Error {
...
pub fn decode<T: Into<VastcoreError>>(error: T) -> Error {
	match error.into() {
		VastcoreError::Decoder(ref dec_err) => rlp(dec_err.clone()),
		_ => Error {
			code: ErrorCode::InternalError,
...
pub fn call(error: CallError) -> Error {
	match error {
		...
		CallError::TransactionNotFound => internal("{}, this should not be the case with vast_call, most likely a bug.", CallError::TransactionNotFound),
	}
}
...
pub fn deprecated<S: Into<String>, T: Into<Option<S>>>(message: T) -> Error {
	Error {
		...
		message: "Mvastod deprecated".into(),
...
pub fn require_experimental(allow_experimental_rpcs: bool, eip: &str) -> Result<(), Error> {
	if allow_experimental_rpcs {
		...
		Err(Error {
			...
			message: format!("This mvastod is not part of the official RPC API yet (EIP-{}). Run with `--jsonrpc-experimental` to enable it.", eip),
			data: Some(Value::String(format!("See EIP: https://eips.vast.org/EIPS/eip-{}", eip))),
		})
	}
}
