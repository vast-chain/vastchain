...
use vast_types::{H256, U64};
use jsonrpc_core::Result;
use v1::types::{Bytes, RecoveredAccount};
use v1::helpers::errors;
use v1::helpers::dispatch::vast_data_hash;
use hash::keccak;

/// helper mvastod for vast_verifySignature
...
mod tests {
	...
	fn sign(should_prefix: bool, data: Vec<u8>, signing_chain_id: Option<u64>) -> (H160, [u8; 32], [u8; 32], U64) {
		let hash = if should_prefix { vast_data_hash(data) } else { keccak(data) };
