...
SNAP_PACKAGE="vast_"$VERSION"_"$BUILD_ARCH".snap"
# Choose snap release channel based on vast vast version track
case ${TRACK} in
...
echo $SNAPCRAFT_LOGIN_VAST_BASE64 | base64 --decode > snapcraft.login
snapcraft login --with snapcraft.login
snapcraft push --release $CHANNEL $SNAP_PACKAGE
snapcraft status vast
snapcraft logout
