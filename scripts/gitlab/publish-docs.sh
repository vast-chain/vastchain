...
clone_repos() {
    ...
    git clone https://github.com/vast-js/jsonrpc.git jsonrpc
    git clone https://github.com/vasttech/wiki.git wiki
    git clone https://github.com/vasttech/vast-config-generator
}
...
update_wiki_docs() {
    ...
    mv vast-config-generator/docs/config.md wiki/Configuring-Vast-Vast.md
}

setup_git() {
    echo "__________Set github__________"
    git config --global user.email "devops-team@vast.io"
    git config --global user.name "Devops Team Vast"
}

set_remote_wiki() {
    git config remote.origin.url "https://${GITHUB_TOKEN}@github.com/vasttech/wiki.git"
}
...
setup_git
clone_repos
mkdir -p "jsonrpc/.vast/$RPC_TRAITS_DIR"
cp $RPC_TRAITS_DIR/*.rs "jsonrpc/.vast/$RPC_TRAITS_DIR"
cd jsonrpc
build_docs
cd ..
cd vast-config-generator
build_config
cd ..
update_wiki_docs
cd wiki
