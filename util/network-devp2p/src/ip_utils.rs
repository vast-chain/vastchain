			...
			match search_gateway(search_options) {
				...
				Ok(gateway) => {
					match gateway.get_external_ip() {
						...
						Ok(external_addr) => {
							match gateway.add_any_port(PortMappingProtocol::TCP, SocketAddrV4::new(local_ip, local_port), 0, "Vast Node/TCP") {
								Err(ref err) => {
									debug!("Port mapping error: {}", err);
								},
								Ok(tcp_port) => {
									match gateway.add_any_port(PortMappingProtocol::UDP, SocketAddrV4::new(local_ip, local_udp_port), 0, "Vast Node/UDP") {
										Err(ref err) => {
											debug!("Port mapping error: {}", err);
										},
										Ok(udp_port) => {
...
/// Port mapping using vaster UPnP or Nat-PMP.
