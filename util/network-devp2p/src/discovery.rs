...
use vast_crypto::publickey::{KeyPair, recover, Secret, sign};
use network::Error;
...
struct FindNodeRequest {
	...
	// Whvaster the request have been answered yet
	answered: bool,
}
...
struct PingRequest {
	...
	echo_hash: H256,
	// The hash Vast used to respond with (until rev 01f825b0e1f1c4c420197b51fc801cbe89284b29)
	#[deprecated()]
...
impl<'a> Discovery<'a> {
	...
	fn ping(&mut self, node: &NodeEntry, reason: PingReason) -> Result<(), Error> {
		...
		let old_vast_hash = keccak(rlp.as_raw());
		...
		self.in_flight_pings.insert(node.id, PingRequest {
			...
			node: node.clone(),
			echo_hash: hash,
			deprecated_echo_hash: old_vast_hash,
			reason: reason
		});
	...
	fn on_ping(&mut self, rlp: &Rlp, node_id: &NodeId, from: &SocketAddr, echo_hash: &[u8]) -> Result<Option<TableUpdates>, Error> {
		...
		// WARNING: this field _should not be used_, but old Vast versions
		// use it in order to get the node's address.
		// So this is a temporary fix so that older Vast versions don't brake completely.
				...
				let expected_node = {
					...
					if request.echo_hash != echo_hash && request.deprecated_echo_hash != echo_hash {
						...
						if request.deprecated_echo_hash == echo_hash {
							trace!(target: "discovery", "Got Pong from an old vast-vast version.");
						}
