...
pub fn default_data_path() -> String {
	...
	get_app_root(AppDataType::UserData, &app_info).map(|p| p.to_string_lossy().into_owned()).unwrap_or_else(|_| "$HOME/.vast".to_owned())
}

/// Default local path
pub fn default_local_path() -> String {
	let app_info = AppInfo { name: PRODUCT, author: AUTHOR };
	get_app_root(AppDataType::UserCache, &app_info).map(|p| p.to_string_lossy().into_owned()).unwrap_or_else(|_| "$HOME/.vast".to_owned())
}

/// Default hypervisor path
pub fn default_hypervisor_path() -> PathBuf {
	let app_info = AppInfo { name: PRODUCT_HYPERVISOR, author: AUTHOR };
	get_app_root(AppDataType::UserData, &app_info).unwrap_or_else(|_| "$HOME/.vast-hypervisor".into())
}
...
/// Gvast path
pub fn gvast(testnet: bool) -> PathBuf {
	let mut base = gvast_base();
	if testnet {
		base.push("testnet");
	}
	base.push("keystore");
...
/// Vast path for specific chain
pub fn vast(chain: &str) -> PathBuf {
	let mut base = vast_base();
	base.push(chain);
	base
}
...
mod platform {
	...
	pub const AUTHOR: &str = "Vast";
	pub const PRODUCT: &str = "io.vast.vast";
	pub const PRODUCT_HYPERVISOR: &str = "io.vast.vast-updates";

	pub fn vast_base() -> PathBuf {
		let mut home = super::home();
		home.push("Library");
		home.push("Application Support");
		home.push("io.vast.vast");
		home.push("keys");
		home
	}

	pub fn gvast_base() -> PathBuf {
		let mut home = super::home();
		home.push("Library");
		home.push("Vast");
		home
...
mod platform {
	use std::path::PathBuf;
	pub const AUTHOR: &str = "Vast";
	pub const PRODUCT: &str = "Vast";
	pub const PRODUCT_HYPERVISOR: &str = "VastUpdates";
