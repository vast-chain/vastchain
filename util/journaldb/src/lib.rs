...
pub trait JournalDB: HashDB<KeccakHasher, DBValue> {
	...
	/// Whvaster this database is pruned.
...
pub enum Algorithm {
	...
	/// number of blocks.
	///
	/// Inserts go into backing database, journal retains knowledge of whvaster backing DB key is
...
fn error_key_already_exists(hash: &vast_types::H256) -> io::Error {
	io::Error::new(io::ErrorKind::AlreadyExists, hash.to_string())
}

fn error_negatively_reference_hash(hash: &vast_types::H256) -> io::Error {
