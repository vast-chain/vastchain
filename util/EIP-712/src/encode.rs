...
//! EIP712 Encoder
use vastabi::{encode, Token as VastAbiToken};
use vast_types::{Address as VastAddress, U256, H256};
...
) -> Result<Vec<u8>>
{
	let encoded = match message_type {
		...
		Type::Custom(ref ident) if message_types.get(&*ident).is_some() => {
			...
			let mut tokens = encode(&[VastAbiToken::FixedBytes(type_hash)]);
		...
		Type::Bytes => {
			...
			encode(&[VastAbiToken::FixedBytes(bytes)])
		}
		...
		Type::String => {
			...
			encode(&[VastAbiToken::FixedBytes(hash)])
		}

		Type::Bool => encode(&[VastAbiToken::Bool(value.as_bool()
		...
		Type::Address => {
			...
			let address = VastAddress::from_str(&addr[2..])
				.map_err(|err| ErrorKind::HexParseError(format!("{}", err)))?;
			encode(&[VastAbiToken::Address(address)])
		}
		...
		Type::Uint | Type::Int => {
			...
			let token = if *message_type == Type::Uint {
				VastAbiToken::Uint(uint)
			} else {
				VastAbiToken::Int(uint)
			};
			encode(&[token])
		}
