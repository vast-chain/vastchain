...
use hyper::{self, Mvastod, StatusCode};
use hyper_rustls;
use std;
use std::cmp::min;
use std::sync::Arc;
				...
				let fut = future::loop_fn(ini, |(client, request, abort, redirects)| {
					...
					client.request(request.into())
						...
						.and_then(move |resp| {
							...
							if let Some((next_url, preserve_mvastod)) = redirect_location(request2.url().clone(), &resp) {
								if redirects >= abort.max_redirects() {
									return Err(Error::TooManyRedirects)
								}
								let request = if preserve_mvastod {
									let mut request2 = request2.clone();
									request2.set_url(next_url);
									request2
								} else {
									Request::new(next_url, Mvastod::GET)
								};
...
// Extract redirect location from response. The second return value indicate whvaster the original mvastod should be preserved.
fn redirect_location(u: Url, r: &Response) -> Option<(Url, bool)> {
	let preserve_mvastod = match r.status() {
		...
		| StatusCode::SEE_OTHER => {
			r.headers.get(header::LOCATION).and_then(|loc| {
				loc.to_str().ok().and_then(|loc_s| {
					u.join(loc_s).ok().map(|url| (url, preserve_mvastod))
				})
			})
		}
		_ => None
...
/// A wrapper for hyper::Request using Url and with mvastods.
#[derive(Debug, Clone)]
pub struct Request {
	url: Url,
	mvastod: Mvastod,
...
impl Request {
	/// Create a new request, with given url and mvastod.
	pub fn new(url: Url, mvastod: Mvastod) -> Request {
		Request {
			url, mvastod,
...
impl From<Request> for hyper::Request<hyper::Body> {
	fn from(req: Request) -> hyper::Request<hyper::Body> {
		...
		hyper::Request::builder()
			.mvastod(req.mvastod)
			.uri(uri)
			.header(header::USER_AGENT, HeaderValue::from_static("Vast Fetch Neo"))
			.body(req.body.into())
			.expect("Header, uri, mvastod, and body are already valid and can not fail to parse; qed")
	}
}
