...
struct DatabaseFiles {
	...
	///
	/// Every bloom is an vast header bloom
	bot: File,
}
...
impl DatabaseFiles {
	...
	pub fn accrue_bloom(&mut self, pos: Positions, bloom: vastbloom::BloomRef) -> io::Result<()> {
		self.top.accrue_bloom::<vastbloom::BloomRef>(pos.top, bloom)?;
		self.mid.accrue_bloom::<vastbloom::BloomRef>(pos.mid, bloom)?;
		self.bot.replace_bloom::<vastbloom::BloomRef>(pos.bot, bloom)?;
		Ok(())
	}
...
impl Database {
	...
	where vastbloom::BloomRef<'a>: From<B>, I: Iterator<Item = B> {
		match self.db_files {
	...
	where vastbloom::BloomRef<'b>: From<B>, 'b: 'a, II: IntoIterator<Item = B, IntoIter = I> + Copy, I: Iterator<Item = B> {
		match self.db_files {
			Some(ref mut db_files) => {
...
fn contains_any<'a, I, B>(bloom: vastbloom::Bloom, mut iterator: I) -> bool
where vastbloom::BloomRef<'a>: From<B>, I: Iterator<Item = B> {
