...
pub use shared::VASTASH_EPOCH_LENGTH;

use common_types::engines::OptimizeFor;
use compute::Light;
use vast_types::{BigEndianHash, U256, U512};
use keccak::H256;
...
impl VastashManager {
	/// Create a new new instance of vastash manager
	pub fn new<T: Into<Option<OptimizeFor>>>(cache_dir: &Path, optimize_for: T, progpow_transition: u64) -> VastashManager {
		VastashManager {
	...
	pub fn compute_light(&self, block_number: u64, header_hash: &H256, nonce: u64) -> ProofOfWork {
		let epoch = block_number / VASTASH_EPOCH_LENGTH;
		let light = {
...
/// Convert an Vastash boundary to its original difficulty. Basically just `f(x) = 2^256 / x`.
pub fn boundary_to_difficulty(boundary: &vast_types::H256) -> U256 {
	difficulty_to_boundary_aux(&boundary.into_uint())
}

/// Convert an Vastash difficulty to the target boundary. Basically just `f(x) = 2^256 / x`.
pub fn difficulty_to_boundary(difficulty: &U256) -> vast_types::H256 {
	BigEndianHash::from_uint(&difficulty_to_boundary_aux(difficulty))
}

fn difficulty_to_boundary_aux<T: Into<U512>>(difficulty: T) -> vast_types::U256 {
...
fn test_lru() {
	...
	let vastash = VastashManager::new(tempdir.path(), None, u64::max_value());
	let hash = [0u8; 32];
	vastash.compute_light(1, &hash, 1);
	vastash.compute_light(50000, &hash, 1);
	assert_eq!(vastash.cache.lock().recent_epoch.unwrap(), 1);
	assert_eq!(vastash.cache.lock().prev_epoch.unwrap(), 0);
	vastash.compute_light(1, &hash, 1);
	assert_eq!(vastash.cache.lock().recent_epoch.unwrap(), 0);
	assert_eq!(vastash.cache.lock().prev_epoch.unwrap(), 1);
	vastash.compute_light(70000, &hash, 1);
	assert_eq!(vastash.cache.lock().recent_epoch.unwrap(), 2);
	assert_eq!(vastash.cache.lock().prev_epoch.unwrap(), 0);
}

#[test]
fn test_difficulty_to_boundary() {
	use vast_types::{H256, BigEndianHash};
	use std::str::FromStr;
...
fn test_difficulty_to_boundary_regression() {
	...
	// https://github.com/vasttech/vast-vast/issues/8397
...
fn test_boundary_to_difficulty_panics_on_zero() {
	boundary_to_difficulty(&vast_types::H256::zero());
}
