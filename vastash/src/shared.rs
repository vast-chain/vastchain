...
pub const VASTASH_EPOCH_LENGTH: u64 = 30000;
pub const VASTASH_CACHE_ROUNDS: usize = 3;
pub const VASTASH_MIX_BYTES: usize = 128;
pub const VASTASH_ACCESSES: usize = 64;
pub const VASTASH_DATASET_PARENTS: u32 = 256;
...
pub fn epoch(block_number: u64) -> u64 {
	block_number / VASTASH_EPOCH_LENGTH
}
...
pub fn get_cache_size(block_number: u64) -> usize {
	// TODO: Memoise
	let mut sz: u64 = CACHE_BYTES_INIT + CACHE_BYTES_GROWTH * (block_number / VASTASH_EPOCH_LENGTH);
...
pub fn get_data_size(block_number: u64) -> usize {
	// TODO: Memoise
	let mut sz: u64 = DATASET_BYTES_INIT + DATASET_BYTES_GROWTH * (block_number / VASTASH_EPOCH_LENGTH);
	sz = sz - VASTASH_MIX_BYTES as u64;
	while !is_prime(sz / VASTASH_MIX_BYTES as u64) {
		sz = sz - 2 * VASTASH_MIX_BYTES as u64;
	}
	sz as usize
}
