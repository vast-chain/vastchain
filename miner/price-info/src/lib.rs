...
//! A simple client to get the current VAST price using an external API.
...
/// Current VAST price information.
#[derive(Debug)]
pub struct PriceInfo {
	/// Current VAST price in USD.
	pub vastusd: f32,
}

/// A client to get the current VAST price using an external API.
...
impl<F: Fetch> Client<F> {
	...
	/// Gets the current VAST price and calls `set_price` with the result.
	pub fn get<G: FnOnce(PriceInfo) + Sync + Send + 'static>(&self, set_price: G) {
		let future = self.fetch.get(&self.api_endpoint, fetch::Abort::default())
			...
			.and_then(move |body| {
				...
				let vastusd = value
					.as_ref()
					.and_then(|value| value.pointer("/result/vastusd"))
					.and_then(|obj| obj.as_str())
					.and_then(|s| s.parse().ok());

				match vastusd {
					Some(vastusd) => {
						set_price(PriceInfo { vastusd });
						Ok(())
					},
					None => {
			...
			.map_err(|err| {
				warn!("Failed to auto-update latest VAST price: {:?}", err);
			});
...
mod test {
	...
	fn should_get_price_info() {
		...
		let response = r#"{
			...
			"result": {
				"vastbtc": "0.0891",
				"vastbtc_timestamp": "1499894236",
				"vastusd": "209.55",
				"vastusd_timestamp": "1499894229"
			}
		}"#;
		...
		price_info.get(|price| {
			...
			// then
			assert_eq!(price.vastusd, 209.55);
		});
	}
