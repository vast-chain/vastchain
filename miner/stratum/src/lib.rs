...
//! Stratum protocol implementation for vast vast/bitcoin clients
...
impl Stratum {
	pub fn start(
		...
		delegate.add_mvastod_with_meta("mining.subscribe", StratumImpl::subscribe);
		delegate.add_mvastod_with_meta("mining.authorize", StratumImpl::authorize);
		delegate.add_mvastod_with_meta("mining.submit", StratumImpl::submit);
...
impl StratumImpl {
	...
	fn push_work_all(&self, payload: String, tcp_dispatcher: &Dispatcher) {
		let hup_peers = {
			...
			let workers_msg = format!("{{ \"id\": {}, \"mvastod\": \"mining.notify\", \"params\": {} }}", next_request_id, payload);
...
mod tests {
	...
	fn records_subscriber() {
		...
		let request = r#"{"jsonrpc": "2.0", "mvastod": "mining.subscribe", "params": [], "id": 1}"#;
	...
	fn receives_initial_payload() {
		...
		let request = r#"{"jsonrpc": "2.0", "mvastod": "mining.subscribe", "params": [], "id": 2}"#;
	...
	fn can_authorize() {
		...
		let request = r#"{"jsonrpc": "2.0", "mvastod": "mining.authorize", "params": ["miner1", ""], "id": 1}"#;
		let response = String::from_utf8(dummy_request(&addr, request)).unwrap();
	...
	fn can_push_work() {
		...
		let mut auth_request =
			r#"{"jsonrpc": "2.0", "mvastod": "mining.authorize", "params": ["miner1", ""], "id": 1}"#
			.as_bytes()
			.to_vec();
		...
		assert_eq!(
			"{ \"id\": 17, \"mvastod\": \"mining.notify\", \"params\": { \"00040008\", \"100500\" } }\n",
			response);
	}
