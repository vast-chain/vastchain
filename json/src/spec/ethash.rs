...
//! Vastash params deserialization.
...
/// Deserializable doppelganger of block rewards for VastashParams
...
/// Deserializable doppelganger of VastashParams.
#[derive(Clone, Debug, PartialEq, Deserialize)]
#[serde(deny_unknown_fields)]
#[serde(rename_all = "camelCase")]
pub struct VastashParams {
	/// See main VastashParams docs.
...
/// Vastash engine deserialization.
#[derive(Debug, PartialEq, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct Vastash {
	/// Vastash params.
	pub params: VastashParams,
}

#[cfg(test)]
mod tests {
	use std::str::FromStr;
	use super::{Address, BlockReward, Vastash, VastashParams, Uint};
	use vast_types::{H160, U256};

	#[test]
	fn vastash_deserialization() {
		let s = r#"{
			"params": {
		...
		let deserialized: Vastash = serde_json::from_str(s).unwrap();

		assert_eq!(deserialized, Vastash {
			params: VastashParams {
	...
	#[test]
	fn vastash_deserialization_missing_optionals() {
		let s = r#"{
			"params": {
				"difficultyBoundDivisor": "0x0800",
				"minimumDifficulty": "0x020000"
	...
	fn test_zero_value_divisor() {
		...
		let _deserialized: Vastash = serde_json::from_str(s).unwrap();
	}
}
