...
use common_types::{
	...
	errors::{VastcoreError, SnapshotError},
	snapshot::RestorationStatus,
};
use client_traits::{ImportBlock, Tick};


use vastcore_private_tx::{self, Importer, Signer};

pub struct PrivateTxService {
	provider: Arc<vastcore_private_tx::Provider>,
}

impl PrivateTxService {
	fn new(provider: Arc<vastcore_private_tx::Provider>) -> Self {
		PrivateTxService {
			provider,
		}
	}
	...
	pub fn provider(&self) -> Arc<vastcore_private_tx::Provider> {
		self.provider.clone()
	}
}
...
impl ClientService {
	...
	pub fn start(
		...
		encryptor: Box<dyn vastcore_private_tx::Encryptor>,
		private_tx_conf: vastcore_private_tx::ProviderConfig,
		private_encryptor_conf: vastcore_private_tx::EncryptorConfig,
		) -> Result<ClientService, VastcoreError>
	{
		...
		let private_keys = Arc::new(vastcore_private_tx::SecretStoreKeys::new(
			client.clone(),
			private_encryptor_conf.key_server_account,
		));
		let provider = Arc::new(vastcore_private_tx::Provider::new(
			client.clone(),
			miner,
			signer,
			encryptor,
			...
			ClientIoMessage::TakeSnapshot(num) => {
				...
				let res = thread::Builder::new().name("Periodic Snapshot".into()).spawn(move || {
					if let Err(e) = snapshot.take_snapshot(&*client, num) {
						match e {
							VastcoreError::Snapshot(SnapshotError::SnapshotAborted) => info!("Snapshot aborted"),
...
mod tests {
	...
	use vastcore_db::NUM_COLUMNS;
	use vastcore::client::ClientConfig;
	use vastcore::miner::Miner;
	use spec;
	...
	fn it_can_be_started() {
		...
		let service = ClientService::start(
			...
			Arc::new(vastcore_private_tx::DummySigner),
			Box::new(vastcore_private_tx::NoopEncryptor),
			Default::default(),
			Default::default(),
		);
