...
use bytes::Bytes;
use bytes::ToPretty;
use vast_types::{Address, H256, H264, U256};
use hash::keccak;
...
use engine::Engine;
use vastcore_miner::pool::VerifiedTransaction;
use vasttrie::Layout;
use evm::Schedule;
use executive_state;
use io::IoChannel;
...
use types::{
	...
	errors::{BlockError, EngineError, VastcoreError, VastcoreResult, ExecutionError, ImportError, SnapshotError},
	filter::Filter,
	header::Header,
...
struct Importer {
	...
	/// Vast engine to be used during import
...
pub struct Client {
	...
	/// Flag changed by `sleep` and `wake_up` mvastods. Not to be confused with `enabled`.
	liveness: AtomicBool,
...
impl Importer {
	...
	fn check_and_lock_block(&self, bytes: &[u8], block: PreverifiedBlock, client: &Client) -> VastcoreResult<(LockedBlock, Option<PendingTransition>)> {
	...
	fn import_old_block(&self, unverified: Unverified, receipts_bytes: &[u8], db: &dyn KeyValueDB, chain: &BlockChain) -> VastcoreResult<()> {
	...
	fn check_epoch_end_signal(
		...
		state_db: &StateDB,
		client: &Client,
	) -> VastcoreResult<Option<PendingTransition>> {
...
impl Client {
	...
	) -> Result<Arc<Client>, VastcoreError> {
	...
	fn prune_ancient(&self, mut state_db: StateDB, chain: &BlockChain) -> Result<(), VastcoreError> {
	...
	///
	/// This mvastod optimizes access patterns for latest block header
...
impl DatabaseRestore for Client {
	...
	fn restore_db(&self, new_db: &str) -> Result<(), VastcoreError> {
...
impl BlockChainReset for Client {
	fn reset(&self, num: u32) -> Result<(), String> {
		...
		// vastcore/blockchain/src/blockchain.rs/Blockchain::is_known_child()
...
impl ImportBlock for Client {
	fn import_block(&self, unverified: Unverified) -> VastcoreResult<H256> {
		if self.chain.read().is_known(&unverified.hash()) {
			return Err(VastcoreError::Import(ImportError::AlreadyInChain));
		}

		let status = self.block_status(BlockId::Hash(unverified.parent_hash()));
		if status == BlockStatus::Unknown {
			return Err(VastcoreError::Block(BlockError::UnknownParent(unverified.parent_hash())));
		}
		...
		match self.importer.block_queue.import(unverified) {
			...
			Err((VastcoreError::Block(e), Some(input))) => {
				self.importer.bad_blocks.report(input.bytes, e.to_string());
				Err(VastcoreError::Block(e))
			},
			Err((VastcoreError::Block(e), None)) => {
...
impl BlockChainClient for Client {
	...
	fn logs(&self, filter: Filter) -> Result<Vec<LocalizedLogEntry>, BlockId> {
		...
		// First, check whvaster `filter.from_block` and `filter.to_block` is on the canon chain. If so, we can use the
		// optimized version.
		let is_canon = |id| {
			match id {
				// If it is referred by number, then it is always on the canon chain.
				&BlockId::Earliest | &BlockId::Latest | &BlockId::Number(_) => true,
				// If it is referred by hash, we see whvaster a hash -> number -> hash conversion gives us the same
				// result.
...
impl IoClient for Client {
	...
	fn queue_ancient_block(&self, unverified: Unverified, receipts_bytes: Bytes) -> VastcoreResult<H256> {
		...
		let hash = unverified.hash();
		{
			...
			if !is_parent_pending && !self.chain.read().is_known(&parent_hash) {
				return Err(VastcoreError::Block(BlockError::UnknownParent(parent_hash)));
			}
		}
...
impl PrepareOpenBlock for Client {
	fn prepare_open_block(&self, author: Address, gas_range_target: (U256, U256), extra_data: Bytes) -> Result<OpenBlock, VastcoreError> {
		let engine = &*self.engine;
		let chain = self.chain.read();
...
impl ImportSealedBlock for Client {
	fn import_sealed_block(&self, block: SealedBlock) -> VastcoreResult<H256> {
		let start = Instant::now();
...
impl SnapshotClient for Client {
	fn take_snapshot<W: SnapshotWriter + Send>(
		...
		at: BlockId,
		p: &RwLock<Progress>,
	) -> Result<(), VastcoreError> {
		if let Snapshotting::Unsupported = self.engine.snapshot_mode() {
			return Err(VastcoreError::Snapshot(SnapshotError::SnapshotsUnsupported));
		}
	...
	fn import_blocks<'a>(
		...
		let do_import = |bytes: Vec<u8>| {
			...
			match self.import_block(block) {
				Err(VastcoreError::Import(ImportError::AlreadyInChain)) => {
...
impl IoChannelQueue {
	...
	pub fn queue<F>(&self, channel: &IoChannel<ClientIoMessage<Client>>, count: usize, fun: F) -> VastcoreResult<()> where
		F: Fn(&Client) + Send + Sync + 'static,
	{
		let queue_size = self.currently_queued.load(AtomicOrdering::Relaxed);
		if queue_size >= self.limit {
			let err_limit = usize::try_from(self.limit).unwrap_or(usize::max_value());
			return Err(VastcoreError::FullQueue(err_limit))
		};
