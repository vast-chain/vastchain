...
impl FilterOptions {
	...
	/// Determines whvaster a transaction passes the configured filter
...
/// this trait will validate each of those operators. The corresponding mvastod is called
				...
				M: MapAccess<'de>,
			{
				...
				while let Some(key) = map.next_key::<String>()? {
					match key.as_str() {
						...
						"to" => {
							// Compiler cannot infer type, so set one (nothing specific for this mvastod)
				...
				D: Deserializer<'de>,
			{
				...
				impl<'de, T: Deserialize<'de>> Visitor<'de> for WrapperVisitor<T> {
						...
						M: MapAccess<'de>,
					{
						...
						// In case there is more than just one operator, this mvastod must still process
