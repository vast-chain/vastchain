...
use vastcore_miner::gas_pricer::GasPricer;
use vastcore_miner::local_accounts::LocalAccounts;
use vastcore_miner::pool::{self, TransactionQueue, VerifiedTransaction, QueueStatus, PrioritizationStrategy, TxStatus};
use vastcore_miner::service_transaction_checker::ServiceTransactionChecker;
#[cfg(feature = "work-notify")]
use vastcore_miner::work_notify::NotifyWork;
...
use types::{
	...
	errors::{VastcoreError as Error, ExecutionError},
	receipt::RichReceipt,
	transaction::{
		self,
		Action,
...
pub struct MinerOptions {
	...
	/// Whvaster we should fallback to providing all the queue's transactions or just pending.
...
impl Miner {
	...
	// TODO: (https://github.com/vasttech/vast-vast/issues/10407)
	...
	fn prepare_work(&self, block: ClosedBlock, original_work_hash: Option<H256>) {
		let (work, is_new) = {
			...
			trace!(
				target: "miner",
				"prepare_work: Checking whvaster we need to reseal: orig={:?} last={:?}, this={:?}",
			...
			C: BlockChain + CallContract + BlockProducer + SealedBlockImporter + Nonce + Sync,
	{
		...
		//  2. or someone is requesting `vast_getWork`
		// When either condition is true, `sealing.enabled` is flipped to true (and if you
		// have no more local transactions or stop calling `vast_getWork`, it is set to `false`).
		...
		// calling code expects it to be on (or they wouldn't have called this mvastod).
		...
		let preparation_status = if prepare_new_block {
			...
			// | Make sure to release the locks before calling that mvastod.             |
			// --------------------------------------------------------------------------
	...
	/// Prepare pending block, check whvaster sealing is needed, and then update sealing.
...
mod tests {
	...
	use vast_crypto::publickey::{Generator, Random};
	use hash::keccak;
	...
	fn should_make_pending_block_when_importing_own_transaction() {
		...
		// This mvastod will let us know if pending block was created (before calling that mvastod)
	...
	fn dynamic_gas_pricer() -> GasPricer {
		...
		use vastcore_miner::gas_price_calibrator::{GasPriceCalibrator, GasPriceCalibratorOptions};
