...
impl From<vastjson::vm::Call> for CallCreate {
	fn from(c: vastjson::vm::Call) -> Self {
		let dst: Option<vastjson::hash::Address> = c.destination.into();
		CallCreate {
			data: c.data.into(),
...
fn do_json_test<H: FnMut(&str, HookType)>(
	...
	let tests = vastjson::test_helpers::vm::Test::load(json_data)
