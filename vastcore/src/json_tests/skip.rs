...
//!
//! Looks in the `vast/tests/test-issues/currents.json` file. This file contains two
...
//! `Somvasting/funTests` would both be matched when `failing` is set to `funTests`.
//!
...
use vastjson::test_helpers::skip::SkipTests;

lazy_static! {
	pub static ref SKIP_TESTS: SkipTests = {
		let skip_data = include_bytes!("../../res/vast/tests-issues/currents.json");
