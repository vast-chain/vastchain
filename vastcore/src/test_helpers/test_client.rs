...
use types::{
	...
	errors::{VastcoreError as Error, VastcoreResult},
...
impl ImportSealedBlock for TestBlockChainClient {
	fn import_sealed_block(&self, _block: SealedBlock) -> VastcoreResult<H256> {
		Ok(H256::zero())
	}
}
...
impl Call for TestBlockChainClient {
	// State will not be used by test client anyway, since all mvastods that accept state are mocked
...
/// test client, since all mvastods that accept state are mocked.
pub struct TestState;

impl StateInfo for TestState {
	fn nonce(&self, _address: &Address) -> vasttrie::Result<U256> { unimplemented!() }
	fn balance(&self, _address: &Address) -> vasttrie::Result<U256> { unimplemented!() }
	fn storage_at(&self, _address: &Address, _key: &H256) -> vasttrie::Result<H256> { unimplemented!() }
	fn code(&self, _address: &Address) -> vasttrie::Result<Option<Arc<Bytes>>> { unimplemented!() }
}
...
impl IoClient for TestBlockChainClient {
	...
	fn queue_ancient_block(&self, unverified: Unverified, _r: Bytes) -> VastcoreResult<H256> {
