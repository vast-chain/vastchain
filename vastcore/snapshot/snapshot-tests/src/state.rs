...
use common_types::{
	...
	errors::{VastcoreError as Error, SnapshotError},
...
fn snap_and_restore() {
	...
	let db_cfg = DatabaseConfig::with_columns(vastcore_db::NUM_COLUMNS);
	...
	let new_db = journaldb::new(db, Algorithm::OverlayRecent,  vastcore_db::COL_STATE);
...
fn get_code_from_prev_chunk() {
	...
	let state_db = journaldb::new(new_db, Algorithm::OverlayRecent, vastcore_db::COL_STATE);
