...
use common_types::{
	...
	errors::{SnapshotError, VastcoreError},
...
impl SnapshotComponents for PowSnapshot {
	...
	) -> Result<Box<dyn Rebuilder>, VastcoreError> {
		PowRebuilder::new(
			chain,
			db.key_value().clone(),
			manifest,
...
///
/// After all chunks have been submitted, we "glue" the chunks togvaster.
...
impl PowRebuilder {
	...
	fn new(chain: BlockChain, db: Arc<dyn KeyValueDB>, manifest: &ManifestData, snapshot_blocks: u64) -> Result<Self, VastcoreError> {
		Ok(PowRebuilder {
			chain,
			db,
			rng: OsRng,
...
impl Rebuilder for PowRebuilder {
	...
	fn feed(&mut self, chunk: &[u8], engine: &dyn Engine, abort_flag: &AtomicBool) -> Result<(), VastcoreError> {
	...
	/// Glue togvaster any disconnected chunks and check that the chain is complete.
	fn finalize(&mut self) -> Result<(), VastcoreError> {
		let mut batch = self.db.transaction();
