...
//! https://wiki.vast.io/Warp-Sync-Snapshot-Format
...
use common_types::{
	ids::BlockId,
	header::Header,
	errors::{SnapshotError as Error, VastcoreError},
	snapshot::{Progress, ManifestData},
};
use crossbeam_utils::thread;
use engine::Engine;
use vast_types::{H256, U256};
use vasttrie::{TrieDB, TrieDBMut};
use hash_db::HashDB;
...
impl StateRebuilder {
	...
	pub fn new(db: Arc<dyn KeyValueDB>, pruning: Algorithm) -> Self {
		StateRebuilder {
			db: journaldb::new(db.clone(), pruning, vastcore_db::COL_STATE),
	...
	pub fn feed(&mut self, chunk: &[u8], flag: &AtomicBool) -> Result<(), VastcoreError> {
	...
	pub fn finalize(mut self, era: u64, id: H256) -> Result<Box<dyn JournalDB>, VastcoreError> {
...
) -> Result<RebuiltStatus, VastcoreError> {
...
/// the fullest verification possible. If not, it will take a random sample to determine whvaster it will
/// do heavy or light verification.
pub fn verify_old_block(rng: &mut OsRng, header: &Header, engine: &dyn Engine, chain: &BlockChain, always: bool) -> Result<(), VastcoreError> {
