...
//! General error types for use in vastcore.

use std::{error, fmt};
use derive_more::{Display, From};
use vast_types::{U256, U512};
use vasttrie::TrieError;
use vast_snappy::InvalidInput;
use vast_crypto::publickey::Error as VastPublicKeyCryptoError;

use errors::{BlockError, EngineError, ImportError, SnapshotError};
use transaction::Error as TransactionError;

/// Vastcore Result
pub type VastcoreResult<T> = Result<T, VastcoreError>;

/// Vastcore Error
#[derive(Debug, Display, From)]
pub enum VastcoreError {
	...
	Engine(EngineError),
	/// Vastkey error."
	#[display(fmt = "Vastkey error: {}", _0)]
	Vastkey(VastPublicKeyCryptoError),
	/// RLP decoding errors
...
impl error::Error for VastcoreError {
	fn source(&self) -> Option<&(dyn error::Error + 'static)> {
		use self::VastcoreError::*;
		match self {
			Io(e) => Some(e),
			StdIo(e) => Some(e),
			Trie(e) => Some(e),
...
impl From<&str> for VastcoreError {
	fn from(s: &str) -> Self {
		VastcoreError::Msg(s.into())
	}
}

impl<E> From<Box<E>> for VastcoreError where VastcoreError: From<E> {
	fn from(err: Box<E>) -> VastcoreError {
		VastcoreError::from(*err)
	}
}
