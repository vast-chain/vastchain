...
use vast_types::{H256, H160, Address, U256, BigEndianHash};
use vastjson;
use vast_crypto::publickey::{Signature, Secret, Public, recover, public_to_address};
use hash::keccak;
...
impl From<vastjson::transaction::Transaction> for SignedTransaction {
	fn from(t: vastjson::transaction::Transaction) -> Self {
		let to: Option<vastjson::hash::Address> = t.to.into();
...
impl From<vastjson::transaction::Transaction> for UnverifiedTransaction {
	fn from(t: vastjson::transaction::Transaction) -> Self {
		let to: Option<vastjson::hash::Address> = t.to.into();
...
impl Transaction {
	...
	pub fn sign(self, secret: &Secret, chain_id: Option<u64>) -> SignedTransaction {
		let sig = vast_crypto::publickey::sign(secret, &self.hash(chain_id))
	...
	/// This mvastod is used in json tests as well as
...
impl UnverifiedTransaction {
	...
	/// Checks whvaster the signature has a low 's' value.
	pub fn check_low_s(&self) -> Result<(), vast_crypto::publickey::Error> {
		if !self.signature().is_low_s() {
			Err(vast_crypto::publickey::Error::InvalidSignature)
		} else {
			Ok(())
		}
	}
	...
	pub fn recover_public(&self) -> Result<Public, vast_crypto::publickey::Error> {
	...
	pub fn verify_basic(&self, check_low_s: bool, chain_id: Option<u64>) -> Result<(), error::Error> {
		if self.is_unsigned() {
			return Err(vast_crypto::publickey::Error::InvalidSignature.into());
		}
		if check_low_s {
			self.check_low_s()?;
		}
	...
	pub fn verify_unordered(self) -> Result<SignedTransaction, vast_crypto::publickey::Error> {
...
impl SignedTransaction {
	/// Try to verify transaction and recover sender.
	pub fn new(transaction: UnverifiedTransaction) -> Result<Self, vast_crypto::publickey::Error> {
		if transaction.is_unsigned() {
			return Err(vast_crypto::publickey::Error::InvalidSignature);
		}
...
mod tests {
	...
	fn signing_eip155_zero_chainid() {
		use vast_crypto::publickey::{Random, Generator};
		...
		let sig = vast_crypto::publickey::sign(&key.secret(), &hash).unwrap();
