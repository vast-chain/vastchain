...
pub trait LightChainClient: Send + Sync {
	...
	/// parent queued prior.
	fn queue_header(&self, header: Header) -> VastcoreResult<H256>;
	...
	/// Query whvaster a block is known.
...
/// Somvasting which can be treated as a `LightChainClient`.
...
impl<T: ChainDataFetcher> Client<T> {
	...
	/// from time to time by a Vast developer in order to update the chain specifications.
	///
	...
	pub fn import_header(&self, header: Header) -> VastcoreResult<H256> {
	...
	pub fn chain_mem_used(&self) -> usize {
		use vast_util_mem::MallocSizeOfExt;
