...
/// Whvaster or not a peer was kept by a handler
...
/// This is an implementation of the light vast network protocol, abstracted
...
impl LightProtocol {
	...
	pub fn make_announcement(&self, io: &dyn IoContext, mut announcement: Announcement) {
		...
		for (peer_id, peer_info) in self.peers.read().iter() {
			...
			if peer_info.sent_head == announcement.head_hash ||
				...
				// https://github.com/vasttech/vast-vast/issues/10419
			...
			let reorg_depth = reorgs_map.entry(peer_info.sent_head)
				.or_insert_with(|| {
					match self.provider.reorg_depth(&announcement.head_hash, &peer_info.sent_head) {
						Some(depth) => depth,
						None => {
							// both values will always originate locally -- this means somvasting
	...
	// is actually decoded:
	//   - check whvaster peer exists
	//   - check whvaster request was made
	//   - check whvaster request kinds match
...
// if somvasting went wrong, figure out how much to punish the peer.
