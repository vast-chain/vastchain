...
mod tests {
	...
	fn should_propagate_service_transaction_to_selected_peers_only() {
		...
		// when peer#1 is Gvast
		insert_dummy_peer(&mut sync, 1, block_hash);
		io.peers_info.insert(1, "Gvast".to_owned());
		// and peer#2 is Vast, accepting service transactions
		insert_dummy_peer(&mut sync, 2, block_hash);
		io.peers_info.insert(2, "Vast-Vast/v2.6.0/linux/rustc".to_owned());
		// and peer#3 is Vast, accepting service transactions
		insert_dummy_peer(&mut sync, 3, block_hash);
		io.peers_info.insert(3, "Vast-Vast/ABCDEFGH/v2.7.3/linux/rustc".to_owned());
	...
	fn should_propagate_service_transaction_is_sent_as_separate_message() {
		...
		// when peer#1 is Vast, accepting service transactions
		insert_dummy_peer(&mut sync, 1, block_hash);
		io.peers_info.insert(1, "Vast-Vast/v2.6.0/linux/rustc".to_owned());
