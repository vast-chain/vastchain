...
use crate::{
	...
	chain::{
		...
		SyncState, VAST_PROTOCOL_VERSION_62, VAST_PROTOCOL_VERSION_63, MAX_NEW_BLOCK_AGE, MAX_NEW_HASHES,
...
use common_types::{
	...
	ids::BlockId,
	errors::{VastcoreError, ImportError, BlockError},
...
impl SyncHandler {
	...
	pub fn on_peer_new_block(sync: &mut ChainSync, io: &mut dyn SyncIo, peer_id: PeerId, r: &Rlp) -> Result<(), DownloaderImportError> {
		...
		match io.chain().import_block(block) {
			...
			Err(VastcoreError::Block(BlockError::UnknownParent(p))) => {
				unknown = true;
	...
	fn on_peer_status(sync: &mut ChainSync, io: &mut dyn SyncIo, peer_id: PeerId, r: &Rlp) -> Result<(), DownloaderImportError> {
		...
		if false
			...
			|| (!warp_protocol && (peer.protocol_version < VAST_PROTOCOL_VERSION_62.0 || peer.protocol_version > VAST_PROTOCOL_VERSION_63.0))
		{
			trace!(target: "sync", "Peer {} unsupported vast protocol ({})", peer_id, peer.protocol_version);
			return Err(DownloaderImportError::Invalid);
		}
