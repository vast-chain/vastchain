...
//! This implementation uses vast protocol v63
//!
...
use crate::{
	VastProtocolInfo as PeerInfoDigest, PriorityTask, SyncConfig, WarpSync, WARP_SYNC_PROTOCOL_ID,
...
use vast_util_mem::{MallocSizeOfExt, malloc_size_of_is_0};
...
/// 63 version of Vast protocol.
pub const VAST_PROTOCOL_VERSION_63: (u8, u8) = (63, 0x11);
/// 62 version of Vast protocol.
pub const VAST_PROTOCOL_VERSION_62: (u8, u8) = (62, 0x11);
/// 1 version of Vast protocol and the packet count.
pub const PAR_PROTOCOL_VERSION_1: (u8, u8) = (1, 0x15);
/// 2 version of Vast protocol (consensus messages added).
pub const PAR_PROTOCOL_VERSION_2: (u8, u8) = (2, 0x16);
/// 3 version of Vast protocol (private transactions messages added).
pub const PAR_PROTOCOL_VERSION_3: (u8, u8) = (3, 0x18);
/// 4 version of Vast protocol (private state sync added).
...
impl ChainSyncApi {
	...
	///
	/// NOTE This mvastod should only handle stuff that can be canceled and would reach other peers
	/// by other means.
	...
	pub fn process_priority_queue(&self, io: &mut dyn SyncIo) {
		...
		let mut work = || {
			...
			match task {
				// NOTE We can't simply use existing mvastods,
...
/// A peer query mvastod for getting a list of peers
enum PeerState {
...
impl ChainSync {
	...
	pub fn status(&self) -> SyncStatus {
		...
		SyncStatus {
			...
			protocol_version: VAST_PROTOCOL_VERSION_63.0,
	...
	/// Start a snapshot with all peers that we are not currently asking somvasting else from. If
	...
	/// Find somvasting to do for a peer. Called for a new peer or when a peer is done with its task.
	...
	fn send_status(&mut self, io: &mut dyn SyncIo, peer: PeerId) -> Result<(), network::Error> {
		...
		let protocol = if warp_protocol { warp_protocol_version } else { VAST_PROTOCOL_VERSION_63.0 };
...
pub mod tests {
	...
	use vastcore::miner::{MinerService, PendingOrdering};
	...
	fn should_add_transactions_to_queue() {
		fn sender(tx: &UnverifiedTransaction) -> Address {
			vast_crypto::publickey::public_to_address(&tx.recover_public().unwrap())
		}
