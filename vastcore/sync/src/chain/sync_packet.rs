...
//! pieces of information togvaster and provides an easy mechanism
//! to convert to/from the packet id values transmitted over the
//! wire.

use crate::api::{VAST_PROTOCOL, WARP_SYNC_PROTOCOL_ID};
use self::SyncPacket::*;
...
enum_from_primitive! {
	...
	/// check whvaster a given packet id is known, and to prevent
...
mod tests {
	...
	fn when_status_packet_then_id_and_protocol_match() {
		...
		assert_eq!(StatusPacket.protocol(), VAST_PROTOCOL);
	}
