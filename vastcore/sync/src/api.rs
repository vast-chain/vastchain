...
use crate::chain::{
	...
	ChainSyncApi, SyncState, SyncStatus as VastSyncStatus, VAST_PROTOCOL_VERSION_62,
	VAST_PROTOCOL_VERSION_63, PAR_PROTOCOL_VERSION_1, PAR_PROTOCOL_VERSION_2,
...
use vastcore_io::TimerToken;
use vastcore_private_tx::PrivateStateDB;
use vast_types::{H256, H512, U256};
use vast_crypto::publickey::Secret;
...
/// Vast sync protocol
pub const VAST_PROTOCOL: ProtocolId = *b"vast";
/// Vast light protocol
...
impl Default for SyncConfig {
	fn default() -> SyncConfig {
		SyncConfig {
			...
			network_id: 1,
			subprotocol_name: VAST_PROTOCOL,
...
pub struct PeerInfo {
	...
	/// Vast protocol info.
	pub vast_info: Option<VastProtocolInfo>,
	/// Light protocol info.
...
/// VastSync initialization parameters.
pub struct Params {
	/// Configuration.
...
/// Vast network protocol handler
pub struct VastSync {
	/// Network service
	network: NetworkService,
	/// Main (vast/par) protocol handler
	vast_handler: Arc<SyncProtocolHandler>,
...
impl VastSync {
	...
	pub fn new(params: Params, connection_filter: Option<Arc<dyn ConnectionFilter>>) -> Result<Arc<VastSync>, Error> {
		...
		{
			// spawn task that constantly updates VastSync.is_major_sync
		...
		let sync = Arc::new(VastSync {
			network: service,
			vast_handler: Arc::new(SyncProtocolHandler {
				sync,
				chain: params.chain,
...
impl SyncProvider for VastSync {
	/// Get sync status
	fn status(&self) -> VastSyncStatus {
		self.vast_handler.sync.status()
	}
	...
	fn peers(&self) -> Vec<PeerInfo> {
		self.network.with_context_eval(self.subprotocol_name, |ctx| {
			...
			let peer_info = self.vast_handler.sync.peer_info(&peer_ids);
	...
	fn transactions_stats(&self) -> BTreeMap<H256, TransactionStats> {
		self.vast_handler.sync.transactions_stats()
	}

	fn sync_notification(&self) -> Notification<SyncState> {
		self.vast_handler.sync.write().sync_notifications()
	}
...
impl ChainNotify for VastSync {
	...
	fn new_blocks(&self, new_blocks: NewBlocks)
	{
		...
		self.network.with_context(self.subprotocol_name, |context| {
			let mut sync_io = NetSyncIo::new(context,
				&*self.vast_handler.chain,
				&*self.vast_handler.snapshot_service,
				&self.vast_handler.overlay,
				self.vast_handler.private_state.clone());
			self.vast_handler.sync.write().chain_new_blocks(
				&mut sync_io,
				&new_blocks.imported,
				&new_blocks.invalid,
		...
		self.network.with_context(self.light_subprotocol_name, |context| {
			...
			let chain_info = self.vast_handler.chain.chain_info();
	...
	fn start(&self) {
			...
			Err((err, listen_address)) => {
				match err.into() {
					Error::Io(ref e) if e.kind() == io::ErrorKind::AddrInUse => {
						warn!("Network port {:?} is already in use, make sure that another instance of an Vast client is not running or change the port using the --port option.", listen_address.expect("Listen address is not set."))
					},
		...
		self.network.register_protocol(self.vast_handler.clone(), self.subprotocol_name, &[VAST_PROTOCOL_VERSION_62, VAST_PROTOCOL_VERSION_63])
			.unwrap_or_else(|e| warn!("Error registering vast protocol: {:?}", e));
		// register the warp sync subprotocol
		self.network.register_protocol(self.vast_handler.clone(), WARP_SYNC_PROTOCOL_ID, &[PAR_PROTOCOL_VERSION_1, PAR_PROTOCOL_VERSION_2, PAR_PROTOCOL_VERSION_3, PAR_PROTOCOL_VERSION_4])
	...
	fn stop(&self) {
		self.vast_handler.snapshot_service.abort_restore();
		self.network.stop();
	}
	...
	fn broadcast(&self, message_type: ChainMessageType) {
		self.network.with_context(WARP_SYNC_PROTOCOL_ID, |context| {
			let mut sync_io = NetSyncIo::new(context,
				...
				self.vast_handler.private_state.clone());
			match message_type {
				ChainMessageType::Consensus(message) => self.vast_handler.sync.write().propagate_consensus_packet(&mut sync_io, message),
				ChainMessageType::PrivateTransaction(transaction_hash, message) =>
					self.vast_handler.sync.write().propagate_private_transaction(&mut sync_io, transaction_hash, PrivateTransactionPacket, message),
				ChainMessageType::SignedPrivateTransaction(transaction_hash, message) =>
					self.vast_handler.sync.write().propagate_private_transaction(&mut sync_io, transaction_hash, SignedPrivateTransactionPacket, message),
				ChainMessageType::PrivateStateRequest(hash) =>
					self.vast_handler.sync.write().request_private_state(&mut sync_io, &hash),
			}
		});
	}

	fn transactions_received(&self, txs: &[UnverifiedTransaction], peer_id: PeerId) {
		let mut sync = self.vast_handler.sync.write();
...
impl ManageNetwork for VastSync {
	fn accept_unreserved_peers(&self) {
	...
	fn stop_network(&self) {
		self.network.with_context(self.subprotocol_name, |context| {
			let mut sync_io = NetSyncIo::new(context,
				...
				&self.vast_handler.overlay,
				self.vast_handler.private_state.clone());
			self.vast_handler.sync.write().abort(&mut sync_io);
		});
...
/// Wrapper around `light_sync::SyncInfo` to expose those mvastods without the concrete type `LightSync`
pub trait LightSyncInfo: Send + Sync {
	...
	/// Whvaster major sync is underway.
