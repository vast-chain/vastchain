...
use common_types::{
	...
	ids::BlockId,
	errors::{VastcoreError, BlockError, ImportError},
};

const MAX_HEADERS_TO_REQUEST: usize = 128;
const MAX_BODIES_TO_REQUEST_LARGE: usize = 128;
const MAX_BODIES_TO_REQUEST_SMALL: usize = 32; // Size request for vast clients prior to 2.4.0
	...
	pub fn collect_blocks(&mut self, io: &mut dyn SyncIo, allow_out_of_order: bool) -> DownloadAction {
		...
		for block_and_receipts in blocks {
			...
			match result {
				...
				Err(VastcoreError::Import(ImportError::AlreadyQueued)) => {
				...
				Err(VastcoreError::Block(BlockError::UnknownParent(_))) if allow_out_of_order => {
					break;
				},
				Err(VastcoreError::Block(BlockError::UnknownParent(_))) => {
					trace_sync!(self, "Unknown new block parent, restarting sync");
					break;
				},
				Err(VastcoreError::Block(BlockError::TemporarilyInvalid(_))) => {
					debug_sync!(self, "Block temporarily invalid: {:?}, restarting sync", h);
					break;
				},
				Err(VastcoreError::FullQueue(limit)) => {
...
mod tests {
	...
	use triehash_vast::ordered_trie_root;
	use common_types::{
