...
use crate::{
	...
	chain::{
		...
		ChainSync, SyncSupplier, VAST_PROTOCOL_VERSION_63, PAR_PROTOCOL_VERSION_4
	},
...
use vastcore::{
	client::{Client as VastcoreClient, ClientConfig},
	test_helpers::{self, TestBlockChainClient},
};
use vastcore::miner::Miner;
use vastcore_io::{IoChannel, IoContext, IoHandler};
...
impl FlushingBlockChainClient for VastcoreClient {
	fn flush(&self) {
		self.flush_queue();
	}
}
...
impl<'p, C> SyncIo for TestIo<'p, C> where C: FlushingBlockChainClient, C: 'p {
	...
	fn vast_protocol_version(&self, _peer: PeerId) -> u8 {
		VAST_PROTOCOL_VERSION_63.0
	}

	fn protocol_version(&self, protocol: &ProtocolId, peer_id: PeerId) -> u8 {
		if protocol == &WARP_SYNC_PROTOCOL_ID { PAR_PROTOCOL_VERSION_4.0 } else { self.vast_protocol_version(peer_id) }
	}
...
pub trait Peer {
	...
	/// Whvaster this peer is done syncing (has no messages to send).
...
pub struct VastPeer<C> where C: FlushingBlockChainClient {
	pub chain: Arc<C>,
	pub miner: Arc<Miner>,
...
impl<C> VastPeer<C> where C: FlushingBlockChainClient {
...
impl<C: FlushingBlockChainClient> Peer for VastPeer<C> {
...
impl TestNet<VastPeer<TestBlockChainClient>> {
	...
	pub fn new_with_config(n: usize, config: SyncConfig) -> Self {
		...
		for _ in 0..n {
			...
			net.peers.push(Arc::new(VastPeer {
	...
	pub fn peer_mut(&mut self, i: usize) -> &mut VastPeer<TestBlockChainClient> {
		Arc::get_mut(&mut self.peers[i]).expect("Arc never exposed externally")
	}
}

impl TestNet<VastPeer<VastcoreClient>> {
	pub fn with_spec<F>(
		n: usize,
		config: SyncConfig,
		spec_factory: F,
...
impl<C: FlushingBlockChainClient> TestNet<VastPeer<C>> {
...
pub struct TestIoHandler {
	pub client: Arc<VastcoreClient>,
	pub private_tx_queued: Mutex<usize>,
}

impl TestIoHandler {
	pub fn new(client: Arc<VastcoreClient>) -> Self {
		TestIoHandler {
			client,
...
impl IoHandler<ClientIoMessage<VastcoreClient>> for TestIoHandler {
	fn message(&self, _io: &IoContext<ClientIoMessage<VastcoreClient>>, net_message: &ClientIoMessage<VastcoreClient>) {
		match *net_message {
...
impl ChainNotify for VastPeer<VastcoreClient> {
