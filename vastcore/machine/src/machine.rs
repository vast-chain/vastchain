...
//! Vast-like state machine definition.
...
use common_types::{
	...
	errors::{EngineError, VastcoreError as Error},
...
use vastcore_call_contract::CallContract;
...
impl Machine {
	...
	pub fn regular(params: CommonParams, builtins: BTreeMap<Address, Builtin>) -> Machine {
		...
		Machine {
			...
			tx_filter,
			vastash_extensions: None,
			schedule_rules: None,
		}
	}

	/// Vast machine with vastash extensions.
	// TODO: either unify or specify to mainnet specifically and include other specific-chain HFs?
	pub fn with_vastash_extensions(params: CommonParams, builtins: BTreeMap<Address, Builtin>, extensions: VastashExtensions) -> Machine {
		let mut machine = Machine::regular(params, builtins);
		machine.vastash_extensions = Some(extensions);
		machine
	}
	...
	/// Get a reference to the vastash-specific extensions.
	pub fn vastash_extensions(&self) -> Option<&VastashExtensions> {
		self.vastash_extensions.as_ref()
	}
	...
	pub fn on_new_block(&self, block: &mut ExecutedBlock) -> Result<(), Error> {
		...
		if let Some(ref vastash_params) = self.vastash_extensions {
			if block.header.number() == vastash_params.dao_hardfork_transition {
				let state = block.state_mut();
				for child in &vastash_params.dao_hardfork_accounts {
					let beneficiary = &vastash_params.dao_hardfork_beneficiary;
					state.balance(child)
	...
	pub fn populate_from_parent(&self, header: &mut Header, parent: &Header, gas_floor_target: U256, gas_ceil_target: U256) {
		...
		if let Some(ref vastash_params) = self.vastash_extensions {
			...
			if header.number() >= vastash_params.dao_hardfork_transition &&
				header.number() <= vastash_params.dao_hardfork_transition + 9 {
	...
	pub fn schedule(&self, block_number: BlockNumber) -> Schedule {
		let mut schedule = match self.vastash_extensions {
	...
	pub fn verify_transaction_basic(&self, t: &UnverifiedTransaction, header: &Header) -> Result<(), transaction::Error> {
		let check_low_s = match self.vastash_extensions {
...
// 2) it will be a nearest (with tendency to increase) multiple of VAST_GAS_LIMIT_DETERMINANT
fn round_block_gas_limit(gas_limit: U256, lower_limit: U256, upper_limit: U256) -> U256 {
	let increased_gas_limit = gas_limit + (VAST_GAS_LIMIT_DETERMINANT - gas_limit % VAST_GAS_LIMIT_DETERMINANT);
	if increased_gas_limit > upper_limit {
		let decreased_gas_limit = increased_gas_limit - VAST_GAS_LIMIT_DETERMINANT;
...
mod tests {
	...
	fn get_default_vastash_extensions() -> VastashExtensions {
		VastashExtensions {
	...
	fn should_disallow_unsigned_transactions() {
		...
		let vastparams = get_default_vastash_extensions();

		let machine = Machine::with_vastash_extensions(
			spec.params().clone(),
			Default::default(),
			vastparams,
		);
	...
	#[test]
	fn vastash_gas_limit_is_multiple_of_determinant() {
		use vast_types::U256;
		...
		assert_eq!(VAST_GAS_LIMIT_DETERMINANT, U256::from(37));
