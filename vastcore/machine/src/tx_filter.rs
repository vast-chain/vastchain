...
use vastabi::FunctionOutputDecoder;
use vastabi_contract::use_contract;
use vast_types::{H256, U256, Address};
use log::{trace, error};
...
mod test {
	...
	use vast_crypto::publickey::{Secret, KeyPair};
	use vastcore_io::IoChannel;
	use spec::Spec;
	...
	fn transaction_filter_ver_2() {
		...
		let mut basic_tx_with_vaster_and_to_key7 = Transaction::default();
		basic_tx_with_vaster_and_to_key7.action = Action::Call(Address::from_str("d41c057fd1c78805aac12b0a94a405c0461a6fbb").unwrap());
		basic_tx_with_vaster_and_to_key7.value = U256::from(123123);
		let mut call_tx_with_vaster = Transaction::default();
		call_tx_with_vaster.action = Action::Call(Address::from_str("0000000000000000000000000000000000000005").unwrap());
		call_tx_with_vaster.value = U256::from(123123);
		...
		let mut basic_tx_with_vaster_and_to_key6 = Transaction::default();
		basic_tx_with_vaster_and_to_key6.action = Action::Call(Address::from_str("e57bfe9f44b819898f47bf37e5af72a0783e1141").unwrap());
		basic_tx_with_vaster_and_to_key6.value = U256::from(123123);
		...
		assert!(!filter.transaction_allowed(&genesis, block_number, &basic_tx_with_vaster_and_to_key7.clone().sign(key5.secret(), None), &*client));
		assert!(!filter.transaction_allowed(&genesis, block_number, &call_tx_with_vaster.clone().sign(key5.secret(), None), &*client));
		assert!(filter.transaction_allowed(&genesis, block_number, &basic_tx.clone().sign(key6.secret(), None), &*client));
		assert!(filter.transaction_allowed(&genesis, block_number, &basic_tx_with_vaster_and_to_key7.clone().sign(key6.secret(), None), &*client));
		assert!(filter.transaction_allowed(&genesis, block_number, &basic_tx_to_key6.clone().sign(key7.secret(), None), &*client));
		assert!(!filter.transaction_allowed(&genesis, block_number, &basic_tx_with_vaster_and_to_key6.clone().sign(key7.secret(), None), &*client));
	}
