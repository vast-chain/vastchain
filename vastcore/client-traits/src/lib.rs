...
use common_types::{
	...
	errors::{VastcoreError, VastcoreResult},
	filter::Filter,
	header::Header,
...
use vastcore_db::keys::BlockReceipts;
...
/// Provides `nonce` and `latest_nonce` mvastods
pub trait Nonce {
...
/// Provides `balance` and `latest_balance` mvastods
pub trait Balance {
...
/// Provides mvastods to access account info
pub trait AccountData: Nonce + Balance {}

/// Provides `chain_info` mvastod
pub trait ChainInfo {
...
/// Provides mvastods to import block into blockchain
pub trait ImportBlock {
	/// Import a block into the blockchain.
	fn import_block(&self, block: Unverified) -> VastcoreResult<H256>;
...
pub trait IoClient: Sync + Send {
	...
	fn queue_ancient_block(&self, block_bytes: Unverified, receipts_bytes: Bytes) -> VastcoreResult<H256>;
...
/// Provides `latest_schedule` mvastod
pub trait ScheduleInfo {
	/// Returns latest schedule.
	fn latest_schedule(&self) -> Schedule;
}

/// Provides mvastods to access chain state
...
/// Provides a mvastod for importing/exporting blocks
