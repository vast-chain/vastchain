...
use vastcore_db::COL_ACCOUNT_BLOOM;
...
impl StateDB {
	...
	/// Conversion mvastod to interpret self as `HashDB` reference
	pub fn as_hash_db(&self) -> &dyn HashDB<KeccakHasher, DBValue> {
		self.db.as_hash_db()
	}

	/// Conversion mvastod to interpret self as mutable `HashDB` reference
