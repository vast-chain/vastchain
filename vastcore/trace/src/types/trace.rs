...
use vast_types::{U256, Address, Bloom, BloomInput};
use vast_bytes::Bytes;
...
/// `Create` mvastod. Distinguish between use of `CREATE` and `CREATE2` opcodes in an action.
#[derive(Debug, Clone, PartialEq)]
pub enum CreationMvastod {
	/// Create
	Create,
	/// Create2
	Create2,
...
impl TryFrom<ActionType> for CreationMvastod {
	type Error = &'static str;
	fn try_from(action_type: ActionType) -> Result<Self, Self::Error> {
		match action_type {
			ActionType::Call => Err("Call cannot be converted to CreationMvastod"),
			ActionType::CallCode => Err("CallCode cannot be converted to CreationMvastod"),
			ActionType::DelegateCall => Err("DelegateCall cannot be converted to CreationMvastod"),
			ActionType::StaticCall => Err("StaticCall cannot be converted to CreationMvastod"),
			ActionType::Create => Ok(CreationMvastod::Create),
			ActionType::Create2 => Ok(CreationMvastod::Create2),
		}
	}
}

impl Encodable for CreationMvastod {
	fn rlp_append(&self, s: &mut RlpStream) {
		let v = match *self {
			CreationMvastod::Create => 0u32,
			CreationMvastod::Create2 => 1,
		};
		Encodable::rlp_append(&v, s);
	}
}

impl Decodable for CreationMvastod {
	fn decode(rlp: &Rlp) -> Result<Self, DecoderError> {
		rlp.as_val().and_then(|v| Ok(match v {
			0u32 => CreationMvastod::Create,
			1 => CreationMvastod::Create2,
			_ => return Err(DecoderError::Custom("Invalid value of CreationMvastod item")),
		}))
	}
}
...
/// the older encodings used in vast-vast versions < 2.7 and 2.7.0.
#[derive(Debug, Clone, Copy, PartialEq)]
...
pub struct Create {
	...
	/// The init code.
	pub init: Bytes,
	/// Creation mvastod (CREATE vs CREATE2).
	#[rlp(default)]
	pub creation_mvastod: Option<CreationMvastod>,
}
...
impl From<ActionParams> for Create {
	fn from(p: ActionParams) -> Self {
		Create {
			...
			creation_mvastod: CreationMvastod::try_from(p.action_type).ok().into(),
		}
	}
}
...
mod tests {
	...
	use rlp_derive::{RlpEncodable, RlpDecodable};
	use super::{Address, Bytes, Call, CallType, Create, CreationMvastod, U256};
	...
	#[test]
	fn test_creation_mvastod_backwards_compatibility() {
