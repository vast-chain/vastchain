...
use vast_types::{H256, U256, Address, BigEndianHash as _};
...
// The same as `call_msg`, but send a `pwasm_vast::gasleft`
// value as `gas` argument to the inner pwasm_vast::call
#[test]
fn call_msg_gasleft() {
	let _ = ::env_logger::try_init();
...
fn keccak() {
	...
	params.gas = U256::from(100_000);
	params.code = Some(Arc::new(code));
	params.data = Some(b"somvasting".to_vec());
	let mut ext = FakeExt::new().with_wasm();
...
// varios blockchain runtime mvastods
#[test]
fn externs() {
	let (gas_left, result) = reqrep_test!(
		"externs.wasm",
...
fn embedded_keccak() {
	...
	code.extend_from_slice(b"somvasting");
