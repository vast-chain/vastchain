...
// NOTE: state tests are found in vastcore/src/executive_state.rs
...
use vasttrie::{TrieDB, Result as TrieResult};
...
///
/// `State` can work togvaster with `StateDB` to share account cache.
///
...
/// * In global but not local - somvasting that was queried from the database,
/// but never modified
/// * In local but not global - somvasting that was just added (e.g. new account)
/// * In both with the same value - somvasting that was changed to a new value,
/// but changed back to a previous block in the same block (same State instance)
/// * In both with different values - somvasting that was overwritten with a
/// new value.
///
...
/// Provides subset of `State` mvastods to query state information
pub trait StateInfo {
...
impl<B: Backend> State<B> {
	...
	/// Determine whvaster an account exists.
	...
	/// Determine whvaster an account exists and if not empty.
	...
	/// Determine whvaster an account exists and has code or non-zero nonce.
	...
	/// Whvaster the base storage root of an account remains unchanged.
	...
	/// Use of this mvastod requires FatDB mode to be able
	...
	/// Differs from existing mvastod by including all storage
	...
	/// Load required account data from the databases. Returns whvaster the cache succeeds.
	#[must_use]
