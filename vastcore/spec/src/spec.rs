...
use clique::Clique;
use engine::Engine;
use vastash_engine::Vastash;
use vast_types::{H256, Bloom, U256, Address};
use vastjson;
...
pub struct SpecParams<'a> {
	...
	/// Whvaster to run slower at the expense of better memory usage, or run faster while using
	/// more
...
impl From<vastjson::spec::HardcodedSync> for SpecHardcodedSync {
	fn from(sync: vastjson::spec::HardcodedSync) -> Self {
		SpecHardcodedSync {
...
fn convert_json_to_spec(
	(address, builtin): (vastjson::hash::Address, vastjson::spec::builtin::Builtin),
...
fn load_from(spec_params: SpecParams, s: vastjson::spec::Spec) -> Result<Spec, Error> {
...
impl Spec {
	// create an instance of an Vast state machine, minus consensus logic.
	fn machine(
		engine_spec: &vastjson::spec::Engine,
		params: CommonParams,
		builtins: BTreeMap<Address, Builtin>,
	) -> Machine {
		if let vastjson::spec::Engine::Vastash(ref vastash) = *engine_spec {
	...
	fn engine(
		...
		match engine_spec {
			vastjson::spec::Engine::Null(null) => Arc::new(NullEngine::new(null.params.into(), machine)),
			vastjson::spec::Engine::Vastash(vastash) => Arc::new(Vastash::new(spec_params.cache_dir, vastash.params.into(), machine, spec_params.optimization_setting)),
			vastjson::spec::Engine::InstantSeal(Some(instant_seal)) => Arc::new(InstantSeal::new(instant_seal.params.into(), machine)),
			vastjson::spec::Engine::InstantSeal(None) => Arc::new(InstantSeal::new(InstantSealParams::default(), machine)),
			vastjson::spec::Engine::BasicAuthority(basic_authority) => Arc::new(BasicAuthority::new(basic_authority.params.into(), machine)),
			vastjson::spec::Engine::Clique(clique) => Clique::new(clique.params.into(), machine)
								.expect("Failed to start Clique consensus engine."),
			vastjson::spec::Engine::AuthorityRound(authority_round) => AuthorityRound::new(authority_round.params.into(), machine)
	...
	pub fn load_machine<R: Read>(reader: R) -> Result<Machine, Error> {
		vastjson::spec::Spec::load(reader)
