...
impl From<Box<vasttrie::TrieError>> for Error {
	fn from(err: Box<vasttrie::TrieError>) -> Self {
		Error::Internal(format!("Internal error: {}", err))
	}
}

impl From<vasttrie::TrieError> for Error {
	fn from(err: vasttrie::TrieError) -> Self {
		Error::Internal(format!("Internal error: {}", err))
	}
}
