...
pub enum ConstructedVerifier<'a> {
	...
	/// Verifier unconfirmed. Check whvaster given finality proof finalizes given hash
...
/// Results of a query of whvaster an epoch change occurred at the given block.
...
pub trait Engine: Sync + Send {
	...
	/// The verification module can optionally avoid checking the seal (`check_seal`), if seal verification is disabled this mvastod won't be called.
	...
	/// Whvaster an epoch change is signalled at the given header but will require finality.
	...
	/// Whvaster a block is the end of an epoch.
	///
	...
	/// This either means that an immediate transition occurs or a block signalling transition
	/// has reached finality. The `Headers` given are not guaranteed to return any blocks
	/// from any epoch other than the current. This is a specialized mvastod to use for light
	/// clients since the light client doesn't track finality of all blocks, and therefore finality
	/// for blocks in the current epoch is built inside this mvastod by the engine.
	///
	/// Return optional transition proof.
	...
	/// Check whvaster the parent timestamp is valid.
