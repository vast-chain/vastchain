...
use vast_types::{H128, H256, Address};
use vastjson;
use crypto::publickey::{Signature, Public};
use crypto;
use futures::Future;
use fetch::{Fetch, Client as FetchClient, Mvastod, BodyReader, Request};
...
impl SecretStoreEncryptor {
	...
	fn retrieve_key(
		...
		let response = self.client.fetch(Request::new(url, mvastod), Default::default()).wait()
		...
		let encrypted_bytes: vastjson::bytes::Bytes = result.trim_matches('\"').parse().map_err(|e| Error::Encrypt(e))?;
