...
extern crate vast_crypto as crypto;
...
use vast_types::{H128, H256, U256, Address, BigEndianHash};
use hash::keccak;
use rlp::*;
...
use machine::{
	executive::{Executive, TransactOptions, contract_address as vastcore_contract_address},
...
use vastcore::client::{Client, Call};
use client_traits::{BlockInfo, ChainNotify};
use vastcore::miner::{self, Miner, MinerService, pool_client::NonceCache};
...
// Source avaiable at https://github.com/vast-contracts/private-tx/blob/master/contracts/PrivateContract.sol
...
impl Provider {
			...
			V: VMTracer,
	{
		...
		let contract_address = contract_address.unwrap_or_else(|| {
			let (new_address, _) = vastcore_contract_address(CreateContractAddress::FromSenderAndNonce, &sender, &nonce, &transaction.data);
			new_address
		});
