...
use types::{
	errors::{VastcoreError, ExecutionError},
...
pub enum Error {
	...
	/// General vastcore error.
	#[display(fmt = "General vastcore error {}", _0)]
	Vastcore(VastcoreError),
...
impl From<VastcoreError> for Error {
	fn from(err: VastcoreError) -> Self {
		Error::Vastcore(err).into()
	}
}
