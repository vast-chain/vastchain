...
use vast_crypto::publickey::{Secret, KeyPair, Signature};
use machine::executive::contract_address;
use hash::keccak;

use vastcore_private_tx::{NoopEncryptor, Provider, ProviderConfig, StoringKeyProvider};
...
fn private_contract() {
	...
	let signer = Arc::new(vastcore_private_tx::KeyPairSigner(vec![key1.clone(), key3.clone(), key4.clone()]));
	...
	let io = vastcore_io::IoChannel::disconnected();
	...
	let signatures: Vec<_> = [&key3, &key4].iter().map(|k|
		Signature::from(vast_crypto::publickey::sign(&k.secret(), &nonced_state_hash).unwrap().into_electrum())).collect();
	...
	let signatures: Vec<_> = [&key4].iter().map(|k|
		Signature::from(vast_crypto::publickey::sign(&k.secret(), &private_state_hash).unwrap().into_electrum())).collect();
...
fn call_other_private_contract() {
	// This test verifies calls private contract mvastods from another one
