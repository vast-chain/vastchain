...
use common_types::{
	...
	errors::{BlockError, VastcoreError as Error},
	snapshot::Snapshotting,
};
use engine::Engine;
use vast_types::{H256, U256};
use vastjson;
use vastash::{self, quick_get_difficulty, slow_hash_block_number, VastashManager};
...
/// Number of blocks in an vastash snapshot.
// make dependent on difficulty increment divisor?
const SNAPSHOT_BLOCKS: u64 = 5000;
/// Maximum number of blocks allowed in an vastash snapshot.
const MAX_SNAPSHOT_BLOCKS: u64 = 30000;

/// Vastash params.
#[derive(Debug, PartialEq)]
pub struct VastashParams {
	/// Minimum difficulty.
...
impl From<vastjson::spec::VastashParams> for VastashParams {
	fn from(p: vastjson::spec::VastashParams) -> Self {
		VastashParams {
			...
			block_reward: p.block_reward.map_or_else(
				...
				|reward| {
					match reward {
						vastjson::spec::BlockReward::Single(reward) => {
							let mut ret = BTreeMap::new();
							ret.insert(0, reward.into());
							ret
						},
						vastjson::spec::BlockReward::Multi(multi) => {
							multi.into_iter()
...
/// Engine using Vastash proof-of-work consensus algorithm, suitable for Vast
/// mainnet chains in the Olympic, Frontier and Homestead eras.
pub struct Vastash {
	vastash_params: VastashParams,
	pow: Arc<VastashManager>,
	machine: Machine,
}

impl Vastash {
	/// Create a new instance of Vastash engine
	pub fn new<T: Into<Option<OptimizeFor>>>(
		...
		let progpow_transition = vastash_params.progpow_transition;

		Vastash {
			vastash_params,
			machine,
			pow: Arc::new(VastashManager::new(
				cache_dir.as_ref(),
				optimize_for.into(),
				progpow_transition
			)),
...
fn verify_block_unordered(pow: &Arc<VastashManager>, header: &Header) -> Result<(), Error> {
	let seal = VastashSeal::parse_seal(header.seal())?;
	...
	let difficulty = vastash::boundary_to_difficulty(&H256(result.value));
...
// TODO [rphmeier]
//
// for now, this is different than Vastash's own epochs, and signal
// "consensus epochs".
// in this sense, `Vastash` is epochless: the same `EpochVerifier` can be used
// for any block in the chain.
// in the future, we might move the Vastash epoch
...
impl Engine for Vastash {
	fn name(&self) -> &str { "Vastash" }
	...
	fn extra_info(&self, header: &Header) -> BTreeMap<String, String> {
		match VastashSeal::parse_seal(header.seal()) {
			Ok(seal) => map![
	...
	fn on_close_block(&self, block: &mut ExecutedBlock, _parent_header: &Header) -> Result<(), Error> {
		...
		let rewards = match self.vastash_params.block_reward_contract {
			Some(ref c) if number >= self.vastash_params.block_reward_contract_transition => {
			...
			_ => {
				...
				let (_, reward) = self.vastash_params.block_reward.iter()
					.rev()
				...
				let eras_rounds = self.vastash_params.ecip1017_era_rounds;
	...
	fn verify_block_basic(&self, header: &Header) -> Result<(), Error> {
		...
		let min_difficulty = self.vastash_params.minimum_difficulty;
		...
		let difficulty = vastash::boundary_to_difficulty(&H256(quick_get_difficulty(
			&header.bare_hash().0,
			seal.nonce.to_low_u64_be(),
			&seal.mix_hash.0,
			header.number() >= self.vastash_params.progpow_transition
		)));
...
impl Vastash {
	fn calculate_difficulty(&self, header: &Header, parent: &Header) -> U256 {
		...
		let difficulty_hardfork = header.number() >= self.vastash_params.difficulty_hardfork_transition;
		let difficulty_bound_divisor = if difficulty_hardfork {
			self.vastash_params.difficulty_hardfork_bound_divisor
		} else {
			self.vastash_params.difficulty_bound_divisor
		};

		let expip2_hardfork = header.number() >= self.vastash_params.expip2_transition;
		let duration_limit = if expip2_hardfork {
			self.vastash_params.expip2_duration_limit
		} else {
			self.vastash_params.duration_limit
		};

		let frontier_limit = self.vastash_params.homestead_transition;
		...
		let mut target = if header.number() < frontier_limit {
			...
			trace!(target: "vastash", "Calculating difficulty parent.difficulty={}, header.timestamp={}, parent.timestamp={}", parent.difficulty(), header.timestamp(), parent.timestamp());
			//block_diff = parent_diff + parent_diff // 2048 * max(1 - (block_timestamp - parent_timestamp) // 10, -99)
			let (increment_divisor, threshold) = if header.number() < self.vastash_params.eip100b_transition {
				(self.vastash_params.difficulty_increment_divisor, 1)
			} else if parent_has_uncles {
				(self.vastash_params.metropolis_difficulty_increment_divisor, 2)
			} else {
				(self.vastash_params.metropolis_difficulty_increment_divisor, 1)
			};
		...
		if header.number() < self.vastash_params.bomb_defuse_transition {
			if header.number() < self.vastash_params.ecip1010_pause_transition {
				let mut number = header.number();
				let original_number = number;
				for (block, delay) in &self.vastash_params.difficulty_bomb_delays {
			...
			} else if header.number() < self.vastash_params.ecip1010_continue_transition {
				let fixed_difficulty = ((self.vastash_params.ecip1010_pause_transition / EXP_DIFF_PERIOD) - 2) as usize;
				target = cmp::max(min_difficulty, target + (U256::from(1) << fixed_difficulty));
			} else {
				let period = ((parent.number() + 1) / EXP_DIFF_PERIOD) as usize;
				let delay = ((self.vastash_params.ecip1010_continue_transition - self.vastash_params.ecip1010_pause_transition) / EXP_DIFF_PERIOD) as usize;
...
mod tests {
	...
	use engine::Engine;
	use vast_types::{H64, H256, U256, Address};
	use vastcore::{
		block::*,
	...
	use super::{Vastash, VastashParams, ecip1017_eras_block_reward};
	...
	fn get_default_vastash_params() -> VastashParams {
		VastashParams {
	...
	fn difficulty_frontier() {
		...
		let vastparams = get_default_vastash_params();
		let tempdir = TempDir::new("").unwrap();
		let vastash = Vastash::new(tempdir.path(), vastparams, machine, None);
		...
		let difficulty = vastash.calculate_difficulty(&header, &parent_header);
	...
	fn difficulty_classic_bomb_delay() {
		let machine = new_homestead_test_machine();
		let vastparams = VastashParams {
			ecip1010_pause_transition: 3000000,
			..get_default_vastash_params()
		};
		let tempdir = TempDir::new("").unwrap();
		let vastash = Vastash::new(tempdir.path(), vastparams, machine, None);
		...
		assert_eq!(
			...
			vastash.calculate_difficulty(&header, &parent_header)
		);
	...
	fn test_extra_info() {
		...
		let mut header = Header::default();
		header.set_seal(vec![rlp::encode(&H256::from_str("b251bd2e0283d0658f2cadfdc8ca619b5de94eca5742725e2e757dd13ed7503d").unwrap()), rlp::encode(&H64::zero())]);
		let info = vastash.extra_info(&header);
