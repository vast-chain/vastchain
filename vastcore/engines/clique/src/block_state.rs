...
impl CliqueBlockState {
	...
	// see https://github.com/vast/go-vast/blob/master/consensus/clique/clique.go#L474
	...
	pub fn apply(&mut self, header: &Header, is_checkpoint: bool) -> Result<Address, Error> {
		...
		if is_checkpoint {
			...
			// will need some memory and might be better for allocation algorithm to decide whvaster to shrink or not
	...
	/// `SystemTime`
	// TODO(niklasad1): refactor this mvastod to be in constructor of `CliqueBlockState` instead.
	...
	/// Returns whvaster the signer is authorized to sign a block
	pub fn is_authorized(&self, author: &Address) -> bool {
		self.signers.contains(author) && !self.recent_signers.contains(author)
	}

	/// Returns whvaster it makes sense to cast the specified vote in the
	...
	// Note this mvastod will always return `true` but it is intended for a uniform `API`
