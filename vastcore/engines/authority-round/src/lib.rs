...
use vast_crypto::publickey::Signature;
use io::{IoContext, IoHandler, TimerToken, IoService};
use itertools::{self, Itertools};
use rand::rngs::OsRng;
use rlp::{encode, Decodable, DecoderError, Encodable, RlpStream, Rlp};
use vast_types::{H256, H520, Address, U128, U256};
...
use common_types::{
	...
	errors::{BlockError, VastcoreError as Error, EngineError},
	ids::BlockId,
...
impl From<vastjson::spec::AuthorityRoundParams> for AuthorityRoundParams {
	fn from(p: vastjson::spec::AuthorityRoundParams) -> Self {
		let map_step_duration = |u: vastjson::uint::Uint| {
		...
		let step_durations: BTreeMap<_, _> = match p.step_duration {
			vastjson::spec::StepDuration::Single(u) =>
				iter::once((0,  map_step_duration(u))).collect(),
			vastjson::spec::StepDuration::Transitions(tr) => {
				if tr.is_empty() {
		...
		let randomness_contract_address = p.randomness_contract_address.map_or_else(BTreeMap::new, |transitions| {
			transitions.into_iter().map(|(vastjson::uint::Uint(block), addr)| {
...
#[derive(Debug)]
struct Step {
	calibrate: bool, // whvaster calibration is enabled.
	inner: AtomicU64,
...
impl EmptyStep {
	...
	fn verify(&self, validators: &dyn ValidatorSet) -> Result<bool, Error> {
		...
		vast_crypto::publickey::verify_address(&correct_proposer, &self.signature.into(), &message)
			.map_err(|e| e.into())
	}

	fn author(&self) -> Result<Address, Error> {
		let message = keccak(empty_step_rlp(self.step, &self.parent_hash));
		let public = vast_crypto::publickey::recover(&self.signature.into(), &message)?;
		Ok(vast_crypto::publickey::public_to_address(&public))
	}
...
fn verify_external(header: &Header, validators: &dyn ValidatorSet, empty_steps_transition: u64) -> Result<(), Error> {
	...
	let is_invalid_proposer = *header.author() != correct_proposer || {
		...
		!vast_crypto::publickey::verify_address(&correct_proposer, &proposer_signature, &header_seal_hash)?
	};
...
impl Engine for AuthorityRound {
	...
	fn verify_block_family(&self, header: &Header, parent: &Header) -> Result<(), Error> {
		...
		// reported as there's no way to tell whvaster the empty step message was never sent or simply not included.
	...
	fn sign(&self, hash: H256) -> Result<Signature, Error> {
		Ok(self.signer.read()
			.as_ref()
			.ok_or(vast_crypto::publickey::Error::InvalidAddress)?
			.sign(hash)?
		)
	}
...
mod tests {
	...
	use vast_types::{Address, H520, H256, U256};
	use vast_crypto::publickey::Signature;
	use common_types::{
		header::Header,
		engines::{Seal, params::CommonParams},
		ids::BlockId,
		errors::{VastcoreError as Error, EngineError},
	...
	fn should_collect_block_reward_transitions() {
		...
		let deserialized: vastjson::spec::AuthorityRound = serde_json::from_str(config).unwrap();
