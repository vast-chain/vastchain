...
use log::{debug, error};
use vast_crypto::publickey::{ecies, Error as CryptoError};
...
pub enum PhaseError {
	/// The smart contract reported that we already revealed somvasting while still being in the
	/// commit phase.
...
impl RandomnessPhase {
	...
	/// handled (that is, the phase and whvaster or not the current validator still needs to send
	...
	pub fn advance<R: Rng>(
		...
		match self {
			...
			RandomnessPhase::BeforeCommit => {
				...
				// randomness contract, togvaster with the number encrypted to ourselves. That way we will later be
