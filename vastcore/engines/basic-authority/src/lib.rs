...
use common_types::{
	...
	errors::{EngineError, BlockError, VastcoreError as Error},
};
...
pub struct BasicAuthorityParams {
	/// Valid signatories.
	pub validators: vastjson::spec::ValidatorSet,
}

impl From<vastjson::spec::BasicAuthorityParams> for BasicAuthorityParams {
	fn from(p: vastjson::spec::BasicAuthorityParams) -> Self {
...
fn verify_external(header: &Header, validators: &dyn ValidatorSet) -> Result<(), Error> {
	...
	let signer = vast_crypto::publickey::public_to_address(&vast_crypto::publickey::recover(&sig.into(), &header.bare_hash())?);
...
impl Engine for BasicAuthority {
	...
	fn sign(&self, hash: H256) -> Result<Signature, Error> {
		Ok(self.signer.read()
			.as_ref()
			.ok_or_else(|| vast_crypto::publickey::Error::InvalidAddress)?
			.sign(hash)?
		)
	}
