...
/// Validator set maintained in a contract, updated using `getValidators` mvastod.
...
use common_types::{
	BlockNumber,
	header::Header,
	errors::{EngineError, VastcoreError, BlockError},
	ids::BlockId,
	log_entry::LogEntry,
...
use vast_types::{H256, U256, Address, Bloom};
...
// TODO: vastabi should be able to generate this.
...
fn decode_first_proof(rlp: &Rlp) -> Result<(Header, Vec<DBValue>), VastcoreError> {
	let header = rlp.val_at(0)?;
	let state_items = rlp.at(1)?
		.iter()
		.map(|x| Ok(x.data()?.to_vec()) )
		.collect::<Result<_, VastcoreError>>()?;
...
fn decode_proof(rlp: &Rlp) -> Result<(Header, Vec<Receipt>), VastcoreError> {
...
impl ValidatorSafeContract {
	...
	// Whvaster the header matches the expected bloom.
	//
	// The expected log should have 3 topics:
	//   1. VASTABI-encoded log name.
...
impl ValidatorSet for ValidatorSafeContract {
	...
	fn on_epoch_begin(&self, _first: bool, _header: &Header, caller: &mut SystemCall) -> Result<(), VastcoreError> {
	...
	fn epoch_set(&self, first: bool, machine: &Machine, _number: BlockNumber, proof: &[u8])
		-> Result<(SimpleList, Option<H256>), VastcoreError>
	{
