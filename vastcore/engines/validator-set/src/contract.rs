...
impl ValidatorSet for ValidatorContract {
	...
	fn on_epoch_begin(&self, first: bool, header: &Header, call: &mut SystemCall) -> Result<(), VastcoreError> {
	...
	fn epoch_set(&self, first: bool, machine: &Machine, number: BlockNumber, proof: &[u8]) -> Result<(SimpleList, Option<H256>), VastcoreError> {
...
mod tests {
	...
	use vast_types::{H520, Address};
	use keccak_hash::keccak;
	use vast_bytes::ToPretty;
	use rlp::encode;
