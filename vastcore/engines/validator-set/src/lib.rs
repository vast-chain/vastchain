...
use vastjson::spec::ValidatorSet as ValidatorSpec;
use machine::Machine;
// The MallocSizeOf derive looks for this in the root
use vast_util_mem as malloc_size_of;
...
pub trait ValidatorSet: Send + Sync + 'static {
	...
	fn on_epoch_begin(&self, _first: bool, _header: &Header, _call: &mut SystemCall) -> Result<(), VastcoreError> {
		Ok(())
	}
	...
	/// Whvaster this block is the last one in its epoch.
	///
	...
	/// Whvaster the given block signals the end of an epoch, but change won't take effect
	/// until finality.
	///
	...
	/// whvaster this header is first in its set.
	///
	/// May fail if the given header doesn't kick off an epoch or
	/// the proof is invalid.
	///
	/// Returns the set, along with a flag indicating whvaster finality of a specific
