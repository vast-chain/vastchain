...
use common_types::errors::VastcoreError;
use vast_types::{H256, U256};
use vast_crypto::publickey::{Signature, recover as ec_recover};
use keccak_hash::keccak;
use log::{warn, trace};
use num::{BigUint, Zero, One};
use vast_bytes::BytesRef;
use vast_crypto::digest;
...
/// on the given input, and `is_active` to determine whvaster the contract is active.
pub struct Builtin {
...
impl Builtin {
	...
	/// Whvaster the builtin is activated at the given block number.
	#[inline]
...
impl TryFrom<vastjson::spec::builtin::Builtin> for Builtin {
	type Error = VastcoreError;

	fn try_from(b: vastjson::spec::builtin::Builtin) -> Result<Self, Self::Error> {
		let native = VastBuiltin::from_str(&b.name)?;
...
impl From<vastjson::spec::builtin::Pricing> for Pricing {
	fn from(pricing: vastjson::spec::builtin::Pricing) -> Self {
		match pricing {
			vastjson::spec::builtin::Pricing::Blake2F { gas_per_round } => {
				Pricing::Blake2F(gas_per_round)
			}
			vastjson::spec::builtin::Pricing::Linear(linear) => {
				Pricing::Linear(Linear {
					base: linear.base,
					word: linear.word,
				})
			}
			vastjson::spec::builtin::Pricing::Modexp(exp) => {
			...
			vastjson::spec::builtin::Pricing::AltBn128Pairing(pricer) => {
			...
			vastjson::spec::builtin::Pricing::AltBn128ConstOperations(pricer) => {
...
impl FromStr for VastBuiltin {
	type Err = VastcoreError;

	fn from_str(name: &str) -> Result<VastBuiltin, Self::Err> {
		match name {
			"identity" => Ok(VastBuiltin::Identity(Identity)),
			"ecrecover" => Ok(VastBuiltin::EcRecover(EcRecover)),
			"sha256" => Ok(VastBuiltin::Sha256(Sha256)),
			"ripemd160" => Ok(VastBuiltin::Ripemd160(Ripemd160)),
			"modexp" => Ok(VastBuiltin::Modexp(Modexp)),
			"alt_bn128_add" => Ok(VastBuiltin::Bn128Add(Bn128Add)),
			"alt_bn128_mul" => Ok(VastBuiltin::Bn128Mul(Bn128Mul)),
			"alt_bn128_pairing" => Ok(VastBuiltin::Bn128Pairing(Bn128Pairing)),
			"blake2_f" => Ok(VastBuiltin::Blake2F(Blake2F)),
			_ => return Err(VastcoreError::Msg(format!("invalid builtin name: {}", name))),
		}
	}
}

impl Implementation for VastBuiltin {
	fn execute(&self, input: &[u8], output: &mut BytesRef) -> Result<(), &'static str> {
		match self {
			VastBuiltin::Identity(inner) => inner.execute(input, output),
			VastBuiltin::EcRecover(inner) => inner.execute(input, output),
			VastBuiltin::Sha256(inner) => inner.execute(input, output),
			VastBuiltin::Ripemd160(inner) => inner.execute(input, output),
			VastBuiltin::Modexp(inner) => inner.execute(input, output),
			VastBuiltin::Bn128Add(inner) => inner.execute(input, output),
			VastBuiltin::Bn128Mul(inner) => inner.execute(input, output),
			VastBuiltin::Bn128Pairing(inner) => inner.execute(input, output),
			VastBuiltin::Blake2F(inner) => inner.execute(input, output),
		}
	}
}
...
mod tests {
	...
	use vast_bytes::BytesRef;
	use super::{
		BTreeMap, Builtin, VastBuiltin, FromStr, Implementation, Linear,
	...
	fn blake2f_cost() {
		let f = Builtin {
			...
			native: VastBuiltin::from_str("blake2_f").unwrap(),
		};
		// 5 rounds
	...
	fn blake2f_cost_on_invalid_length() {
		let f = Builtin {
			pricer: map![0 => Pricing::Blake2F(123)],
			native: VastBuiltin::from_str("blake2_f").expect("known builtin"),
		};
	...
	fn blake2_f_is_err_on_invalid_length() {
		let blake2 = VastBuiltin::from_str("blake2_f").unwrap();
		// Test vector 1 and expected output from https://github.com/vast/EIPs/blob/master/EIPS/eip-152.md#test-vector-1
	...
	fn blake2_f_is_err_on_invalid_length_2() {
		let blake2 = VastBuiltin::from_str("blake2_f").unwrap();
		// Test vector 2 and expected output from https://github.com/vast/EIPs/blob/master/EIPS/eip-152.md#test-vector-2
	...
	fn blake2_f_is_err_on_bad_finalization_flag() {
		let blake2 = VastBuiltin::from_str("blake2_f").unwrap();
		// Test vector 3 and expected output from https://github.com/vast/EIPs/blob/master/EIPS/eip-152.md#test-vector-3
	...
	fn blake2_f_zero_rounds_is_ok_test_vector_4() {
		let blake2 = VastBuiltin::from_str("blake2_f").unwrap();
		// Test vector 4 and expected output from https://github.com/vast/EIPs/blob/master/EIPS/eip-152.md#test-vector-4
	...
	fn blake2_f_test_vector_5() {
		let blake2 = VastBuiltin::from_str("blake2_f").unwrap();
		// Test vector 5 and expected output from https://github.com/vast/EIPs/blob/master/EIPS/eip-152.md#test-vector-5
	...
	fn blake2_f_test_vector_6() {
		let blake2 = VastBuiltin::from_str("blake2_f").unwrap();
		// Test vector 6 and expected output from https://github.com/vast/EIPs/blob/master/EIPS/eip-152.md#test-vector-6
	...
	fn blake2_f_test_vector_7() {
		let blake2 = VastBuiltin::from_str("blake2_f").unwrap();
		// Test vector 7 and expected output from https://github.com/vast/EIPs/blob/master/EIPS/eip-152.md#test-vector-7
	...
	fn blake2_f_test_vector_8() {
		let blake2 = VastBuiltin::from_str("blake2_f").unwrap();
		// Test vector 8 and expected output from https://github.com/vast/EIPs/blob/master/EIPS/eip-152.md#test-vector-8
	...
	#[test]
	fn identity() {
		let f = VastBuiltin::from_str("identity").unwrap();
		let i = [0u8, 1, 2, 3];
	...
	#[test]
	fn sha256() {
		let f = VastBuiltin::from_str("sha256").unwrap();
		let i = [0u8; 0];
	...
	#[test]
	fn ripemd160() {
		let f = VastBuiltin::from_str("ripemd160").unwrap();
		let i = [0u8; 0];
	...
	#[test]
	fn ecrecover() {
		let f = VastBuiltin::from_str("ecrecover").unwrap();
	...
	fn modexp() {
		let f = Builtin {
			...
			native: VastBuiltin::from_str("modexp").unwrap(),
		};
	...
	fn bn128_add() {
		...
		let f = Builtin {
			...
			native: VastBuiltin::from_str("alt_bn128_add").unwrap(),
		};
	...
	fn bn128_mul() {
		...
		let f = Builtin {
			pricer: map![0 => Pricing::Linear(Linear { base: 0, word: 0 })],
			native: VastBuiltin::from_str("alt_bn128_mul").unwrap(),
		};
	...
	fn builtin_pairing() -> Builtin {
		Builtin {
			pricer: map![0 => Pricing::Linear(Linear { base: 0, word: 0 })],
			native: VastBuiltin::from_str("alt_bn128_pairing").unwrap(),
		}
	}
	...
	fn from_unknown_linear() {
		let _ = VastBuiltin::from_str("foo").unwrap();
	}
	...
	fn is_active() {
		...
		let b = Builtin {
			...
			native: VastBuiltin::from_str("identity").unwrap(),
		};
	...
	#[test]
	fn from_json() {
		let b = Builtin::try_from(vastjson::spec::Builtin {
