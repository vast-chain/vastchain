...
use vastash::{VastashParams, Vastash};
use vast_types::U256;
...
fn vastash_params() -> VastashParams {
	VastashParams {
...
fn build_vastash() -> Vastash {
	let machine = new_constantinople_test_machine();
	let vastash_params = vastash_params();
...
fn block_verification(c: &mut Criterion) {
	...
	let block = Unverified::from_rlp(rlp_8481476.clone()).expect(PROOF);
	let preverified = verification::verify_block_unordered(block, &vastash, true).expect(PROOF);
