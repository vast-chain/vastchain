...
use common_types::{
	...
	errors::{BlockError, VastcoreError as Error, ImportError},
...
use log::{debug, trace};
use vast_util_mem::{MallocSizeOf, MallocSizeOfExt};
...
pub struct VerifierSettings {
	/// Whvaster to scale amount of verifiers according to load.
...
impl<K: Kind, C> VerificationQueue<K, C> {
	...
	//
	// TODO: #11403 - rework `VastcoreError::Block` to include raw bytes of the error cause
	...
	pub fn collect_garbage(&self) {
		...
		// when deciding whvaster to change the number of verifiers.
		#[cfg(not(test))]
...
mod tests {
	...
	use vastcore::test_helpers::{get_good_dummy_block_seq, get_good_dummy_block};
	use vastcore::client::Client;
	use vast_bytes::Bytes;
	use common_types::{
		errors::{VastcoreError, ImportError},
	...
	fn returns_error_for_duplicates() {
		...
		match duplicate_import {
			Err(e) => {
				match e {
					(VastcoreError::Import(ImportError::AlreadyQueued), _) => {},
