...
use common_types::{
	BlockNumber,
	header::Header,
	errors::{VastcoreError as Error, BlockError},
...
/// Phase 2 verification. Perform costly checks such as transaction signatures and block nonce for vastash.
...
pub(crate) fn verify_header_params(header: &Header, engine: &dyn Engine, check_seal: bool) -> Result<(), Error> {
	...
	if let Some(ref ext) = engine.machine().vastash_extensions() {
...
mod tests {
	...
	fn test_verify_block() {
		...
		match basic_test(&create_test_block_with_data(&bad_header, &eip86_transactions, &good_uncles), engine) {
			Err(Error::Transaction(ref e)) if e == &vast_crypto::publickey::Error::InvalidSignature.into() => (),
