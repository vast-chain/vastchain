...
pub enum Error {
	...
	/// Consensus is temporary unreachable. Means that somvasting is currently blocking us from either forming
...
impl Error {
	...
	pub fn is_non_fatal(&self) -> bool {
		match *self {
			...
			Error::VastKey(_) | Error::Serde(_) | Error::Hyper(_) | Error::Database(_) | Error::Internal(_) | Error::Io(_) => false,
		}
	}
}
...
impl fmt::Display for Error {
	fn fmt(&self, f: &mut fmt::Formatter) -> Result<(), fmt::Error> {
		match *self {
			...
			Error::VastKey(ref e) => write!(f, "cryptographic error {}", e),
...
impl From<crypto::publickey::Error> for Error {
	fn from(err: crypto::publickey::Error) -> Self {
		Error::VastKey(err.into())
	}
}

impl From<crypto::Error> for Error {
	fn from(err: crypto::Error) -> Self {
		Error::VastKey(err.to_string())
	}
}
