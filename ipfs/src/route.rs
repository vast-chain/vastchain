...
impl IpfsHandler {
	/// Route path + query string to a specialized mvastod
	...
	fn route_cid(&self, cid: &str) -> Result<Out> {
		...
		match cid.codec {
			Codec::VastBlock => self.block(hash),
			Codec::VastBlockList => self.block_list(hash),
			Codec::VastTx => self.transaction(hash),
			Codec::VastStateTrie => self.state_trie(hash),
...
mod tests {
	...
	fn cid_route_block_list() {
		...
		// `vast-block-list` with Keccak-256
	...
	fn cid_route_state_trie() {
		...
		// `vast-state-trie` with Keccak-256
