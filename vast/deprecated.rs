...
pub fn find_deprecated(args: &Args) -> Vec<Deprecated> {
	...
	if args.arg_vasterbase.is_some() {
		result.push(Deprecated::Replaced("--vasterbase", "--author"));
	}
	...
	if args.cmd_dapp {
		result.push(Deprecated::Removed("vast dapp"));
	}
...
mod tests {
	...
	fn test_find_deprecated() {
		...
		assert_eq!(find_deprecated(&{
			...
			args.arg_vasterbase = Some(Default::default());
			...
			Deprecated::Replaced("--vasterbase", "--author"),
