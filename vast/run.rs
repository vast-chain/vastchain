...
use vastcore::client::{Client, DatabaseCompactionProfile};
use vastcore::miner::{self, stratum, Miner, MinerService, MinerOptions};
use snapshot::{self, SnapshotConfiguration};
use spec::SpecParams;
use verification::queue::VerifierSettings;
use vastcore_logger::{Config as LogConfig, RotatingLogger};
use vastcore_service::ClientService;
use futures::Stream;
...
use vast_version::version;
use vastcore_private_tx::{ProviderConfig, EncryptorConfig, SecretStoreEncryptor};
use params::{
	...
	where Cr: Fn(String) + 'static + Send
{
	...
	let mut config = light_client::Config {
		...
		chain_column: ::vastcore_db::COL_LIGHT_CHAIN,
		verify_full: true,
	...
	*sync_handle.write() = Arc::downgrade(&light_sync);
	...
	let deps_for_rpc_apis = Arc::new(rpc_apis::LightDependencies {
		...
		fetch,
		gvast_compatibility: cmd.gvast_compatibility,
		...
		Rr: Fn() + 'static + Send
{
	...
	let store = {
		...
		let store = ::local_store::create(db.key_value().clone(), ::vastcore_db::COL_NODE_INFO, node_info);
...
/// Vast client currently executing in background threads.
///
