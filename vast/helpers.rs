...
use vastcore::client::{DatabaseCompactionProfile, ClientConfig};
use vastcore::miner::{PendingSet, Penalization};
...
// TODO: should we bring it back to vast-types?
...
fn to_seconds(s: &str) -> Result<u64, String> {
	let bad = |_| {
		format!("{}: Invalid duration given. See vast --help for more information.", s)
	};
...
/// Returns default gvast ipc path.
pub fn gvast_ipc_path(testnet: bool) -> String {
	// Windows path should not be hardcoded here.
	// Instead it should be a part of path::vast
	if cfg!(windows) {
		return r"\\.\pipe\gvast.ipc".to_owned();
	}

	if testnet {
		path::vast::with_testnet("gvast.ipc").to_str().unwrap().to_owned()
	} else {
		path::vast::with_default("gvast.ipc").to_str().unwrap().to_owned()
	}
}

/// Formats and returns vast ipc path.
pub fn vast_ipc_path(base: &str, path: &str, shift: u16) -> String {
...
pub fn default_network_config() -> ::sync::NetworkConfiguration {
	...
	NetworkConfiguration {
		...
		client_version: ::vast_version::version(),
	}
}
...
pub fn execute_upgrades(
	...
	match upgrade(&dirs.path) {
		...
		Err(e) => {
			return Err(format!("Error upgrading vast data: {:?}", e));
		},
		_ => {},
	}
...
mod tests {
	...
	use tempdir::TempDir;
	use vast_types::U256;
	use vastcore::miner::PendingSet;
	use vastkey::Password;
	use types::{
		ids::BlockId,
		client_types::Mode,
	};
	use super::{to_duration, to_mode, to_block_id, to_u256, to_pending_set, to_address, to_addresses, to_price, gvast_ipc_path, to_bootnodes, join_set, password_from_file};
...
"#).unwrap();
	...
	#[test]
	#[cfg(windows)]
	fn test_gvast_ipc_path() {
		assert_eq!(gvast_ipc_path(true), r"\\.\pipe\gvast.ipc".to_owned());
		assert_eq!(gvast_ipc_path(false), r"\\.\pipe\gvast.ipc".to_owned());
	}
	...
	fn test_gvast_ipc_path() {
		use path;
		assert_eq!(gvast_ipc_path(true), path::vast::with_testnet("gvast.ipc").to_str().unwrap().to_owned());
		assert_eq!(gvast_ipc_path(false), path::vast::with_default("gvast.ipc").to_str().unwrap().to_owned());
	}
