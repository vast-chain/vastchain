...
use hash::keccak;
use vast_types::{U256, H256, Address};
use vast_version::{version_data, version};
use bytes::Bytes;
use ansi_term::Colour;
use sync::{NetworkConfiguration, validate_node_url, self};
use vast_crypto::publickey::{Secret, Public};
use vastcore::miner::{stratum, MinerOptions};
use snapshot::SnapshotConfiguration;
use miner::pool;
use verification::queue::VerifierSettings;

use rpc::{IpcConfiguration, HttpConfiguration, WsConfiguration};
use vast_rpc::NetworkSettings;
use cache::CacheConfig;
use helpers::{to_duration, to_mode, to_block_id, to_u256, to_pending_set, to_price, gvast_ipc_path, vast_ipc_path, to_bootnodes, to_addresses, to_address, to_queue_strategy, to_queue_penalization};
...
use vastcore_private_tx::{ProviderConfig, EncryptorConfig};
...
use account::{AccountCmd, NewAccount, ListAccounts, ImportAccounts, ImportFromGvastAccounts};
use snapshot_cmd::{self, SnapshotCommand};
use network::{IpFilter, NatType};

const DEFAULT_MAX_PEERS: u16 = 50;
const DEFAULT_MIN_PEERS: u16 = 25;
pub const VASTERSCAN_VAST_PRICE_ENDPOINT: &str = "https://api.vasterscan.io/api?module=stats&action=vastprice";
...
/// Configuration for the Vast client.
#[derive(Debug, PartialEq)]
...
impl Configuration {
	...
	///
	/// ```
	/// let _cfg = vast_vast::Configuration::parse_cli(&["--light", "--chain", "kovan"]).unwrap();
	/// ```
	...
	pub(crate) fn into_command(self) -> Result<Execute, String> {
		...
		let gvast_compatibility = self.args.flag_gvast;
		...
		} else if self.args.flag_import_gvast_keys {
				let account_cmd = AccountCmd::ImportFromGvast(
				ImportFromGvastAccounts {
					spec: spec,
					to: dirs.keys,
	...
	fn author(&self) -> Result<Address, String> {
		to_address(self.args.arg_vasterbase.clone().or(self.args.arg_author.clone()))
	}
	...
	fn gas_pricer_config(&self) -> Result<GasPricerConfig, String> {
		fn wei_per_gas(usd_per_tx: f32, usd_per_vast: f32) -> U256 {
			let wei_per_usd: f32 = 1.0e18 / usd_per_vast;
		...
		if "auto" == self.args.arg_usd_per_vast {
			Ok(GasPricerConfig::Calibrated {
				usd_per_tx: usd_per_tx,
				recalibration_period: to_duration(self.args.arg_price_update_period.as_str())?,
				api_endpoint: VASTERSCAN_VAST_PRICE_ENDPOINT.to_string(),
			})
		} else if let Ok(usd_per_vast_parsed) = to_price(&self.args.arg_usd_per_vast) {
			let wei_per_gas = wei_per_gas(usd_per_tx, usd_per_vast_parsed);

			info!(
				"Using a fixed conversion rate of Ξ1 = {} ({} wei/gas)",
				Colour::White.bold().paint(format!("US${:.2}", usd_per_vast_parsed)),
			...
			Ok(GasPricerConfig::Calibrated {
				usd_per_tx: usd_per_tx,
				recalibration_period: to_duration(self.args.arg_price_update_period.as_str())?,
				api_endpoint: self.args.arg_usd_per_vast.clone(),
			})
		}
	}
	...
	fn net_config(&self) -> Result<NetworkConfiguration, String> {
		...
		ret.client_version = {
			...
			if !self.args.arg_identity.is_empty() {
				// Insert name after the "Vast-Vast/" at the beginning of version string.
	...
	fn ipc_path(&self) -> String {
		if self.args.flag_gvast {
			gvast_ipc_path(self.args.flag_testnet)
		} else {
			vast_ipc_path(
...
mod tests {
	...
	use tempdir::TempDir;
	use vastcore::miner::MinerOptions;
	...
	fn test_command_account_list() {
		let args = vec!["vast", "account", "list"];
		let conf = parse(&args);
	...
	fn test_command_account_import() {
		let args = vec!["vast", "account", "import", "my_dir", "another_dir"];
		let conf = parse(&args);
	...
	fn test_command_wallet_import() {
		let args = vec!["vast", "wallet", "import", "my_wallet.json", "--password", "pwd"];
		let conf = parse(&args);
	...
	fn test_command_blockchain_import() {
		let args = vec!["vast", "import", "blockchain.json"];
		let conf = parse(&args);
	...
	fn test_command_blockchain_export() {
		let args = vec!["vast", "export", "blocks", "blockchain.json"];
		let conf = parse(&args);
	...
	fn test_command_state_export() {
		let args = vec!["vast", "export", "state", "state.json"];
		let conf = parse(&args);
	...
	fn test_command_blockchain_export_with_custom_format() {
		let args = vec!["vast", "export", "blocks", "--format", "hex", "blockchain.json"];
		let conf = parse(&args);
		assert_eq!(conf.into_command().unwrap().cmd, Cmd::Blockchain(BlockchainCmd::Export(ExportBlockchain {
			spec: Default::default(),
			cache_config: Default::default(),
	...
	fn test_command_signer_new_token() {
		let args = vec!["vast", "signer", "new-token"];
		let conf = parse(&args);
		...
		assert_eq!(conf.into_command().unwrap().cmd, Cmd::SignerToken(WsConfiguration {
			...
			origins: Some(vec!["vast://*".into(),"chrome-extension://*".into(), "moz-extension://*".into()]),
			hosts: Some(vec![]),
	...
	fn test_ws_max_connections() {
		let args = vec!["vast", "--ws-max-connections", "1"];
		let conf = parse(&args);
	...
	fn should_parse_mining_options() {
		...
		// when
		let conf0 = parse(&["vast"]);
		let conf2 = parse(&["vast", "--tx-queue-strategy", "gas_price"]);
	...
	fn should_fail_on_force_reseal_and_reseal_min_period() {
		let conf = parse(&["vast", "--chain", "dev", "--force-sealing", "--reseal-min-period", "0"]);
	...
	fn should_parse_updater_options() {
		// when
		let conf0 = parse(&["vast", "--auto-update", "all", "--no-consensus", "--auto-update-delay", "300"]);
		let conf1 = parse(&["vast", "--auto-update=xxx"]);
	...
	fn should_parse_network_settings() {
		...
		// when
		let conf = parse(&["vast", "--testnet", "--identity", "testname"]);
	...
	#[test]
	fn should_parse_rpc_settings_with_gvast_compatiblity() {
		// given
		fn assert(conf: Configuration) {
			...
			assert_eq!(conf.rpc_apis(), "web3,vast".to_owned());
		}

		// when
		let conf1 = parse(&["vast", "-j",
						 "--jsonrpc-port", "8000",
						 "--jsonrpc-interface", "all",
						 "--jsonrpc-cors", "*",
						 "--jsonrpc-apis", "web3,vast"
						 ]);
		let conf2 = parse(&["vast", "--rpc",
							"--rpcport", "8000",
							"--rpcaddr", "all",
							"--rpccorsdomain", "*",
							"--rpcapi", "web3,vast"
	...
	fn should_parse_rpc_hosts() {
		...
		// when
		let conf0 = parse(&["vast"]);
		let conf1 = parse(&["vast", "--jsonrpc-hosts", "none"]);
		let conf2 = parse(&["vast", "--jsonrpc-hosts", "all"]);
		let conf3 = parse(&["vast", "--jsonrpc-hosts", "vast.io,somvasting.io"]);

		// then
		assert_eq!(conf0.rpc_hosts(), Some(Vec::new()));
		assert_eq!(conf1.rpc_hosts(), Some(Vec::new()));
		assert_eq!(conf2.rpc_hosts(), None);
		assert_eq!(conf3.rpc_hosts(), Some(vec!["vast.io".into(), "somvasting.io".into()]));
	}
	...
	fn should_parse_ipfs_hosts() {
		...
		// when
		let conf0 = parse(&["vast"]);
		let conf1 = parse(&["vast", "--ipfs-api-hosts", "none"]);
		let conf2 = parse(&["vast", "--ipfs-api-hosts", "all"]);
		let conf3 = parse(&["vast", "--ipfs-api-hosts", "vast.io,somvasting.io"]);

		// then
		assert_eq!(conf0.ipfs_hosts(), Some(Vec::new()));
		assert_eq!(conf1.ipfs_hosts(), Some(Vec::new()));
		assert_eq!(conf2.ipfs_hosts(), None);
		assert_eq!(conf3.ipfs_hosts(), Some(vec!["vast.io".into(), "somvasting.io".into()]));
	}
	...
	fn should_parse_ipfs_cors() {
		...
		// when
		let conf0 = parse(&["vast"]);
		let conf1 = parse(&["vast", "--ipfs-api-cors", "*"]);
		let conf2 = parse(&["vast", "--ipfs-api-cors", "http://vast.io,http://somvasting.io"]);

		// then
		assert_eq!(conf0.ipfs_cors(), Some(vec![]));
		assert_eq!(conf1.ipfs_cors(), None);
		assert_eq!(conf2.ipfs_cors(), Some(vec!["http://vast.io".into(),"http://somvasting.io".into()]));
	}
	...
	fn should_parse_ui_configuration() {
		...
		// when
		let conf0 = parse(&["vast", "--ui-path=signer"]);
		let conf1 = parse(&["vast", "--ui-path=signer", "--ui-no-validation"]);
		let conf2 = parse(&["vast", "--ui-path=signer", "--ui-port", "3123"]);
		let conf3 = parse(&["vast", "--ui-path=signer", "--ui-interface", "test"]);
		let conf4 = parse(&["vast", "--ui-path=signer", "--force-ui"]);

		// then
		assert_eq!(conf0.directories().signer, "signer".to_owned());

		assert!(conf1.ws_config().unwrap().hosts.is_some());
		assert_eq!(conf1.ws_config().unwrap().origins, Some(vec!["vast://*".into(), "chrome-extension://*".into(), "moz-extension://*".into()]));
	...
	fn should_not_bail_on_empty_line_in_reserved_peers() {
		...
		let args = vec!["vast", "--reserved-peers", filename.to_str().unwrap()];
	...
	fn test_mining_preset() {
		let args = vec!["vast", "--config", "mining"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
	...
	fn test_non_standard_ports_preset() {
		let args = vec!["vast", "--config", "non-standard-ports"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
	...
	fn test_insecure_preset() {
		let args = vec!["vast", "--config", "insecure"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
				...
				// "web3,vast,net,personal,vast,vast_set,traces,rpc,vast_accounts");
	...
	fn test_dev_insecure_preset() {
		let args = vec!["vast", "--config", "dev-insecure"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
				assert_eq!(c.net_settings.chain, "dev");
	...
	fn test_override_preset() {
		let args = vec!["vast", "--config", "mining", "--min-peers=99"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
	...
	fn test_identity_arg() {
		let args = vec!["vast", "--identity", "Somebody"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
				assert_eq!(c.name, "Somebody");
				assert!(c.net_conf.client_version.starts_with("Vast-Vast/Somebody/"));
			}
			_ => panic!("Should be Cmd::Run"),
		}
	}
	...
	fn should_apply_ports_shift() {
		...
		// when
		let conf0 = parse(&["vast", "--ports-shift", "1", "--stratum"]);
		let conf1 = parse(&["vast", "--ports-shift", "1", "--jsonrpc-port", "8544"]);
	...
	fn should_resolve_external_nat_hosts() {
		// Ip works
		let conf = parse(&["vast", "--nat", "extip:1.1.1.1"]);
		...
		let conf = parse(&["vast", "--nat", "extip:192.168.1.1:123"]);
		assert_eq!(conf.net_addresses().unwrap().1.unwrap().ip().to_string(), "192.168.1.1");
		assert_eq!(conf.net_addresses().unwrap().1.unwrap().port(), 30303);

		// Hostname works
		let conf = parse(&["vast", "--nat", "extip:vast.org"]);
		assert!(conf.net_addresses().unwrap().1.is_some());
		assert_eq!(conf.net_addresses().unwrap().1.unwrap().port(), 30303);

		// Hostname works, garbage at the end is discarded
		let conf = parse(&["vast", "--nat", "extip:vast.org:whatever bla bla 123"]);
		assert!(conf.net_addresses().unwrap().1.is_some());
		assert_eq!(conf.net_addresses().unwrap().1.unwrap().port(), 30303);

		// Garbage is error
		let conf = parse(&["vast", "--nat", "extip:blabla"]);
	...
	fn should_expose_all_servers() {
		...
		// when
		let conf0 = parse(&["vast", "--unsafe-expose"]);
	...
	#[test]
	fn allow_ips() {
		let all = parse(&["vast", "--allow-ips", "all"]);
		let private = parse(&["vast", "--allow-ips", "private"]);
		let block_custom = parse(&["vast", "--allow-ips", "-10.0.0.0/8"]);
		let combo = parse(&["vast", "--allow-ips", "public 10.0.0.0/8 -1.0.0.0/8"]);
		let ipv6_custom_public = parse(&["vast", "--allow-ips", "public fc00::/7"]);
		let ipv6_custom_private = parse(&["vast", "--allow-ips", "private -fc00::/7"]);
	...
	fn should_use_correct_cache_path_if_base_is_set() {
		...
		let std = parse(&["vast"]);
		let base = parse(&["vast", "--base-path", "/test"]);
	...
	fn should_respect_only_max_peers_and_default() {
		let args = vec!["vast", "--max-peers=50"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
	...
	fn should_respect_only_min_peers_and_greater_than_default() {
		let args = vec!["vast", "--min-peers=500"];
		let conf = Configuration::parse_cli(&args).unwrap();
		match conf.into_command().unwrap().cmd {
			Cmd::Run(c) => {
	...
	fn should_parse_secretstore_cors() {
		...
		// when
		let conf0 = parse(&["vast"]);
		let conf1 = parse(&["vast", "--secretstore-http-cors", "*"]);
		let conf2 = parse(&["vast", "--secretstore-http-cors", "http://vast.io,http://somvasting.io"]);

		// then
		assert_eq!(conf0.secretstore_cors(), Some(vec![]));
		assert_eq!(conf1.secretstore_cors(), None);
		assert_eq!(conf2.secretstore_cors(), Some(vec!["http://vast.io".into(),"http://somvasting.io".into()]));
	}
}
