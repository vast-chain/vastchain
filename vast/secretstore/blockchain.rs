...
use vast_types::{H256, Address};
use parking_lot::RwLock;
use types::{
	ids::BlockId as VastcoreBlockId,
...
use vastcore::miner::{Miner, MinerService};
use vast_crypto::publickey::Error as VastKeyError;
use sync::SyncProvider;
use registrar::RegistrarClient;
use vastcore_secretstore::{BlockId, BlockNumber, SecretStoreChain, NewBlocksNotify, SigningKeyPair, ContractAddress, Filter};
...
fn into_vastcore_block_id(id: BlockId) -> VastcoreBlockId {
	match id {
		BlockId::Hash(hash) => VastcoreBlockId::Hash(hash),
		BlockId::Number(number) => VastcoreBlockId::Number(number),
		BlockId::Earliest => VastcoreBlockId::Earliest,
		BlockId::Latest => VastcoreBlockId::Latest,
	}
}
...
/// This trust is guaranteed by return result in get_trusted mvastod (if it's not trusted, None is returned)
...
impl SecretStoreChain for TrustedClient {
	...
	fn transact_contract(&self, contract: Address, tx_data: Bytes) -> Result<(), VastKeyError> {
		let client = self.client.upgrade().ok_or_else(|| VastKeyError::Custom("cannot submit tx when client is offline".into()))?;
		let miner = self.miner.upgrade().ok_or_else(|| VastKeyError::Custom("cannot submit tx when miner is offline".into()))?;
		...
		miner.import_own_transaction(&*client, signed.into())
			.map_err(|e| VastKeyError::Custom(format!("failed to import tx: {}", e)))
	}
		...
		match *address {
			...
			ContractAddress::Registry => self.get_trusted().and_then(|client|
				self.get_confirmed_block_hash()
					.and_then(|block| {
						client.get_address(registry_name, VastcoreBlockId::Hash(block))
							.unwrap_or(None)
					})
			),
		}
	...
	fn call_contract(&self, block_id: BlockId, contract_address: Address, data: Bytes) -> Result<Bytes, String> {
		if let Some(client) = self.get_trusted() {
			client.call_contract(into_vastcore_block_id(block_id), contract_address, data)
		} else {
	...
	fn block_hash(&self, id: BlockId) -> Option<H256> {
		if let Some(client) = self.get_trusted() {
			client.block_hash(into_vastcore_block_id(id))
		} else {
			None
		}
	}
	...
	fn block_number(&self, id: BlockId) -> Option<BlockNumber> {
		if let Some(client) = self.get_trusted() {
			client.block_number(into_vastcore_block_id(id))
		} else {
			None
		}
	}
	...
	fn retrieve_last_logs(&self, filter: Filter) -> Option<Vec<RawLog>> {
		...
		self.logs(BlockchainFilter {
			from_block: VastcoreBlockId::Hash(first_block),
			to_block: VastcoreBlockId::Hash(confirmed_block),
