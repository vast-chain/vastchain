...
//! Secret store server's launcher, contains required configuration parameters and launch mvastod
...
mod server {
	...
	fn into_service_contract_address(address: ContractAddress) -> vastcore_secretstore::ContractAddress {
		match address {
			ContractAddress::Registry => vastcore_secretstore::ContractAddress::Registry,
			ContractAddress::Address(address) => vastcore_secretstore::ContractAddress::Address(address),
		}
	}

	/// Key server
	pub struct KeyServer {
		_key_server: Box<dyn vastcore_secretstore::KeyServer>,
	}

	impl KeyServer {
		/// Create new key server
		pub fn new(mut conf: Configuration, deps: Dependencies, executor: Executor) -> Result<Self, String> {
			let self_secret: Arc<dyn vastcore_secretstore::SigningKeyPair> = match conf.self_secret.take() {
				Some(NodeSecretKey::Plain(secret)) => Arc::new(vastcore_secretstore::PlainNodeKeyPair::new(
			...
			let mut cconf = vastcore_secretstore::ServiceConfiguration {
				listener_address: if conf.http_enabled { Some(vastcore_secretstore::NodeAddress {
				...
				cluster_config: vastcore_secretstore::ClusterConfiguration {
					listener_address: vastcore_secretstore::NodeAddress {
						address: conf.interface.clone(),
						port: conf.port,
					},
					nodes: conf.nodes.into_iter().map(|(p, (ip, port))| (p, vastcore_secretstore::NodeAddress {
						address: ip,
						port: port,
					})).collect(),
			...
			let db = vastcore_secretstore::open_secretstore_db(&conf.data_path)?;
			let trusted_client = TrustedClient::new(self_secret.clone(), deps.client, deps.sync, deps.miner);
			let key_server = vastcore_secretstore::start(trusted_client, self_secret, cconf, db, executor)
