...
impl str::FromStr for SpecType {
	...
	fn from_str(s: &str) -> Result<Self, Self::Err> {
		let spec = match s {
			"vast" | "vast"  | "foundation" | "mainnet" => SpecType::Foundation,
			...
			"vastercore" => SpecType::VasterCore,
...
impl fmt::Display for SpecType {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		f.write_str(match *self {
			...
			SpecType::VasterCore => "vastercore",
...
impl SpecType {
	pub fn spec<'a, T: Into<SpecParams<'a>>>(&self, params: T) -> Result<Spec, String> {
		...
		match *self {
			...
			SpecType::VasterCore => Ok(spec::new_vastercore(params)),
...
impl Default for GasPricerConfig {
	fn default() -> Self {
		GasPricerConfig::Calibrated {
			...
			api_endpoint: configuration::VASTERSCAN_VAST_PRICE_ENDPOINT.to_string(),
		}
	}
}
...
mod tests {
	...
	fn test_spec_type_parsing() {
		assert_eq!(SpecType::Foundation, "vast".parse().unwrap());
		assert_eq!(SpecType::Foundation, "vast".parse().unwrap());
		...
		assert_eq!(SpecType::VasterCore, "vastercore".parse().unwrap());
	...
	fn test_spec_type_display() {
		...
		assert_eq!(format!("{}", SpecType::VasterCore), "vastercore");
