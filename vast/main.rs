...
extern crate vast_vast;
extern crate parking_lot;
extern crate vast_daemonize;
extern crate ansi_term;

#[cfg(windows)] extern crate winapi;
extern crate vastcore_logger;
...
use vastcore_logger::setup_log;
use vast_vast::{start, ExecutionAction};
use vast_daemonize::AsHandle;
use parking_lot::{Condvar, Mutex};

const PLEASE_RESTART_EXIT_CODE: i32 = 69;
const VAST_EXECUTABLE_NAME: &str = "vast";
...
#[cfg(windows)]
fn global_cleanup() {
	// We need to clean up all sockets before spawning another Vast process. This makes sure everything is cleaned up.
...
// Starts vast binary installed via `vast-updater` and returns the code it exits with.
fn run_vast() -> Result<(), Error> {
	global_init();
...
struct ExitStatus {
	/// Whvaster the program panicked.
	panicking: bool,
	/// Whvaster the program should exit.
	should_exit: bool,
	/// Whvaster the program should restart.
	should_restart: bool,
	/// If a restart happens, whvaster a new chain spec should be used.
	spec_name_override: Option<String>,
}

// Run `locally installed version` of vast (i.e, not installed via `vast-updater`)
...
fn main_direct(force_can_restart: bool) -> i32 {
	...
	let mut conf = {
		...
		vast_vast::Configuration::parse_cli(&args).unwrap_or_else(|e| e.exit())
	};
	...
	// see: `https://github.com/vasttech/vast-daemonize/pull/13` for more info
	let handle = if let Some(pid) = conf.args.arg_daemon_pid_file.clone() {
		...
		match vast_daemonize::daemonize(pid) {
			Ok(h) => Some(h),
			Err(e) => {
				error!(
					"{}",
...
fn main() {
	...
	// the user has specified to run its originally installed binary (not via `vast-updater`)
	...
	// the binary is named `vast`
	let same_name = exe_path
		.as_ref()
		.map_or(false, |p| {
			p.file_stem().map_or(false, |n| n == VAST_EXECUTABLE_NAME)
		});
	...
	if !force_direct && !development && same_name {
		// Try to run the latest installed version of `vast`,
		// Upon failure it falls back to the locally installed version of `vast`
