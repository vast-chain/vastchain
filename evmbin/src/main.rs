...
//! Vast EVM Interpreter Binary.
//!
//! ## Overview
//!
//! The Vast EVM interpreter binary is a tool in the Vast
//! Vast toolchain. It is an EVM implementation for Vast that
//! is used to run a standalone version of the EVM interpreter.
//!
//! ## Usage
//!
//! The evmbin tool is not distributed with regular Vast releases
//! so you need to build it from source and run it like so:
//!
//! ```bash
//! cargo build -p evmbin --release
//! ./target/release/vast-evm --help
//! ```
...
use vastcore::{json_tests, test_helpers::TrieSpec};
use spec;
...
Usage:
    vast-evm state-test <file> [--chain CHAIN --only NAME --json --std-json --std-dump-json --std-out-only --std-err-only]
...
fn run_state_test(args: Args) {
	use vastjson::test_helpers::state::Test;
...
mod tests {
	...
	use docopt::Docopt;
	use vastcore::test_helpers::TrieSpec;
	use vastjson::test_helpers::state::State;
	use serde::Deserialize;
